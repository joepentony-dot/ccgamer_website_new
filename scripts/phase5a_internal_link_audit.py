...
def detect_redirect(html: str) -> bool:
    return bool(
        ...
        or re.search(r"\b(?:window\.)?location\.href\s*=", html, re.I)
    )
...
def family(relative: str, game_slugs: set[str]) -> str:
    prefixes = (
...
def main() -> None:
    ...
    for path in sorted(path for path in ROOT.rglob("*.html") if included_html(path)):
        ...
        record = {
            "file": relative, "route": route, "family": family(relative, game_slugs),
            "title": strip_tags(title_match.group(1)) if title_match else "",
            "robots": robots, "indexable": "noindex" not in robots.lower(),
    ...
    def target_record(route: str) -> dict | None:
        ...
        if not options:
            return None
        exact = [item for item in options if item["route"] == normalise_route(route)]
        if exact:
            options = exact
        options.sort(key=lambda item: (
            item["canonical_path"] != normalise_route(route), item["redirect"],
    ...
    for source in records:
        ...
        for match in ANCHOR_RE.finditer(source["html"]):
            ...
            route = resolve_href(source["route"], href)
            if route is None or Path(urlparse(route).path).suffix.lower() not in PAGE_EXTENSIONS:
                continue
            text = strip_tags(match.group("body"))
            if text.lower().strip(" .:;!?›»→") in VAGUE_TEXT:
                vague_anchors.append({"source": source["file"], "target": route, "text": text})
            ...
            if target is None:
                broken.add((source["file"], route))
                if breadcrumb or adjacent:
                    navigation_issues.append({
                        "source": source["file"], "target": route, "kind": "broken",
                        "context": "breadcrumb" if breadcrumb else "adjacent",
                    })
                continue
            source["outgoing"].add(route)
            ...
            issues = []
            if target["redirect"]:
                redirects.add((source["file"], route)); redirect_destinations.add(route); issues.append("redirect")
            if not target["indexable"]:
                noindex.add((source["file"], route)); noindex_destinations.add(route); issues.append("noindex")
            if target["canonical_path"] and target["canonical_path"] != normalise_route(route):
                aliases.add((source["file"], route)); alias_destinations.add(route); issues.append("noncanonical-alias")
            if (breadcrumb or adjacent) and issues:
                navigation_issues.append({
                    "source": source["file"], "target": route, "kind": ",".join(issues),
                    "context": "breadcrumb" if breadcrumb else "adjacent",
                })
    ...
    summary = {
        "public_html_pages": len(records), "indexable_pages": len(indexable),
        "noindex_pages": len(records) - len(indexable),
        ...
        "redirect_destination_count": len(redirect_destinations),
    ...
    evidence = {
        ...
        "link_issues": {
            ...
            "redirect_edges": [{"source": a, "target": b} for a, b in sorted(redirects)],
    ...
    md = [
        ...
        f"| Redirect destinations receiving internal links | **{summary['redirect_destination_count']}** |",
    ...
    md.extend(["", "### Broken, redirect, noindex and alias link samples", ""])
    issue_samples = (
    ...
    md.extend(["", "## Hub and parent discovery checks", ""])
