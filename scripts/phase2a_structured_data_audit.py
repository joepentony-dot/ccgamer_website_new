...
The audit examines public repository HTML and the current generators. It writes a
concise Markdown report plus a detailed JSON inventory under docs/seo-baseline/.
It does not alter public pages, game data, sitemaps or generator output.
"""
...
ISO_DURATION_RE = re.compile(r"^P(?=\d|T\d)(?:\d+Y)?(?:\d+M)?(?:\d+D)?(?:T(?:\d+H)?(?:\d+M)?(?:\d+(?:\.\d+)?S)?)?$")
...
def validate_video_object(obj: dict, canonical: str) -> tuple[list[str], list[str]]:
    ...
    for value in values:
        if value and not is_absolute_http(value):
    ...
    if canonical and obj.get("url") and normalise_url_path(str(obj.get("url"))) != normalise_url_path(canonical):
...
def validate_breadcrumb(obj: dict) -> tuple[list[str], list[str]]:
    ...
    if not isinstance(items, list) or len(items) < 2:
        critical.append("BreadcrumbList must contain at least two ListItem entries")
        return critical, warnings
...
def validate_video_game(obj: dict, canonical: str) -> tuple[list[str], list[str]]:
    ...
    if obj.get("author") and obj.get("publisher") and obj.get("author") == obj.get("publisher"):
        warnings.append("VideoGame author duplicates publisher; authorship should represent the creator, not automatically the publisher")
    aggregate = obj.get("aggregateRating")
    if isinstance(aggregate, dict) and str(aggregate.get("ratingCount") or "").strip() == "1":
        warnings.append("AggregateRating uses ratingCount 1; a single editorial score should usually be represented as Review instead")
    return critical, warnings
...
def main() -> None:
    ...
    for path in html_files:
        ...
        for block_index, (_, raw_block) in enumerate(blocks, start=1):
            ...
            try:
                ...
                continue
            page_objects.extend(flatten_schema(payload))
        ...
        for obj in page_objects:
            ...
            for issue in critical:
                schema_critical.append({"file": relative, "type": object_types, "issue": issue})
            for issue in warnings:
                schema_warnings.append({"file": relative, "type": object_types, "issue": issue})
        ...
        if indexable and is_canonical_game:
    ...
    generator_path = ROOT / "scripts" / "generate-slug-pages.js"
    ...
    generator_findings = {
        ...
        "canonical_template_contains_jsonld": "application/ld+json" in generator.split("function buildCanonicalHtml", 1)[-1].split("function writeTextFileIfChanged", 1)[0],
    }

    pages_with_video = sum(1 for page in pages if "VideoObject" in page["schema_types"])
    video_required_gaps = sum(
        1 for item in schema_critical if "VideoObject" in item["type"]
    )
    breadcrumb_critical = sum(
    ...
    summary = {
        ...
        "pages_with_video_object": pages_with_video,
        "video_object_required_property_gaps": video_required_gaps,
        "breadcrumb_critical_issues": breadcrumb_critical,
    ...
    if type_counter:
        for schema_type, count in type_counter.most_common():
            md.append(f"- `{schema_type}`: **{count}**")
    else:
    ...
    if invalid_blocks:
        for item in invalid_blocks[:20]:
            md.append(f"- `{item['file']}` block {item['block']}: {item['error']}")
    else:
        md.append("None detected.")

    md.extend(["", "### Critical schema issues", ""])
    if schema_critical:
        for item in schema_critical[:30]:
    ...
    if metadata_issues:
        for item in metadata_issues[:30]:
            md.append(f"- `{item['file']}` — {item['issue']}")
