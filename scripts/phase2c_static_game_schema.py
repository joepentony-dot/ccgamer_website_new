...
def metadata_snapshot(path: Path) -> dict[str, str]:
    ...
    return {
        ...
        "description": capture(r"<meta\b(?=[^>]*\bname\s*=\s*[\"']description[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "canonical": capture(r"<link\b(?=[^>]*\brel\s*=\s*[\"']canonical[\"'])[^>]*\bhref\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "og_url": capture(r"<meta\b(?=[^>]*\bproperty\s*=\s*[\"']og:url[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "og_title": capture(r"<meta\b(?=[^>]*\bproperty\s*=\s*[\"']og:title[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "og_description": capture(r"<meta\b(?=[^>]*\bproperty\s*=\s*[\"']og:description[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "og_image": capture(r"<meta\b(?=[^>]*\bproperty\s*=\s*[\"']og:image[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "twitter_title": capture(r"<meta\b(?=[^>]*\bname\s*=\s*[\"']twitter:title[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "twitter_description": capture(r"<meta\b(?=[^>]*\bname\s*=\s*[\"']twitter:description[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "twitter_image": capture(r"<meta\b(?=[^>]*\bname\s*=\s*[\"']twitter:image[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
        "twitter_url": capture(r"<meta\b(?=[^>]*\bname\s*=\s*[\"']twitter:url[\"'])[^>]*\bcontent\s*=\s*[\"']([^\"']*)[\"'][^>]*>"),
    }
...
def update_generator() -> bool:
    ...
    expected_artifacts = r'''function getExpectedPageArtifacts(game, slug, validation) {
    const title = stripHtml(game.title || "Game");
    const platformLong = detectPlatform(game);
    const metaPlatform = platformLong === "Amiga" ? "Amiga" : "Commodore";
    const metaDescription = `${title} on ${metaPlatform} — screenshots, manual, downloads and video.`;
    ...
    text = replace_section(text, "function getExpectedPageArtifacts(", "function getCanonicalRewriteReason(", expected_artifacts, "expected page artifacts")
    ...
    text = replace_section(text, "function getRedirectStubRewriteReason(", "function toTokenList(", redirect_reason, "redirect-stub policy")
    ...
    text = replace_section(text, "function buildVideoGameSchema(", "function buildVideoObjectSchema(", videogame_builder, "VideoGame builder")
...
function serializeSchemaForHtml(schema) {
    ...
    text = replace_section(text, "function buildBreadcrumbSchema(", "function buildCanonicalHtml(", breadcrumb_graph, "breadcrumb and graph builders")
...
`;
    ...
    text = replace_section(text, "function buildCanonicalHtml(", "function writeTextFileIfChanged(", canonical_builder, "canonical HTML builder")
...
def main() -> None:
    ...
    if len(games) != EXPECTED_GAME_COUNT or len(set(slugs)) != EXPECTED_GAME_COUNT:
        raise RuntimeError(f"Expected {EXPECTED_GAME_COUNT} unique games, found records={len(games)} unique_slugs={len(set(slugs))}")
    ...
    missing = [path.relative_to(ROOT).as_posix() for path in canonical_files + redirect_files if not path.exists()]
    if missing:
    ...
    if before_video_games not in (0, EXPECTED_GAME_COUNT):
        raise RuntimeError(f"Unexpected VideoGame baseline: {before_video_games}")
    if before_breadcrumbs not in (0, EXPECTED_GAME_COUNT):
        raise RuntimeError(f"Unexpected BreadcrumbList baseline: {before_breadcrumbs}")

    metadata_before = {path.relative_to(ROOT).as_posix(): metadata_snapshot(path) for path in canonical_files}
    redirects_before = {path.relative_to(ROOT).as_posix(): sha256(path) for path in redirect_files}
    protected_before = {relative: sha256(ROOT / relative) for relative in PROTECTED_FILES}

    generator_changed = update_generator()
    run("node", "--check", "scripts/generate-slug-pages.js")
    run("node", "scripts/generate-slug-pages.js")

    metadata_after = {path.relative_to(ROOT).as_posix(): metadata_snapshot(path) for path in canonical_files}
    metadata_changes = sorted(path for path in metadata_before if metadata_before[path] != metadata_after[path])
    if metadata_changes:
        raise RuntimeError(f"Phase 2C altered existing metadata on {len(metadata_changes)} pages: {metadata_changes[:20]}")

    redirect_changes = sorted(relative for relative, before_hash in redirects_before.items() if sha256(ROOT / relative) != before_hash)
    if redirect_changes:
        raise RuntimeError(f"Phase 2C altered legacy redirect stubs: {redirect_changes[:20]}")

    protected_changes = sorted(relative for relative, before_hash in protected_before.items() if sha256(ROOT / relative) != before_hash)
    ...
    for slug, path in zip(slugs, canonical_files):
        ...
        if len(video_games) != 1 or len(breadcrumbs) != 1 or videos:
            failures.append({"slug": slug, "issue": f"graph types VideoGame={len(video_games)} BreadcrumbList={len(breadcrumbs)} VideoObject={len(videos)}"})
            continue
        ...
        if video_game.get("url") != canonical:
            failures.append({"slug": slug, "issue": "VideoGame URL mismatch"})
        if video_game.get("@id") != f"{canonical}#game":
            failures.append({"slug": slug, "issue": "VideoGame @id mismatch"})
    ...
    summary = {
        ...
        "video_game_objects_before": before_video_games,
        "video_game_objects_after": EXPECTED_GAME_COUNT,
        "breadcrumb_objects_before": before_breadcrumbs,
...
Phase 2C connects the existing game-page generator to conservative static Schema.org markup.
...
| Pages with static `VideoGame` | **{before_video_games}** | **{EXPECTED_GAME_COUNT}** |
| Pages with static `BreadcrumbList` | **{before_breadcrumbs}** | **{EXPECTED_GAME_COUNT}** |
| Static `VideoObject` entries added | **0** | **0** |
| Existing metadata fields changed | — | **0** |
| Legacy redirect stubs changed | — | **0** |
| Protected files changed | — | **0** |

## Schema format

Each canonical `/games/<slug>/` wrapper now contains one JSON-LD block with a Schema.org `@graph` containing:

- one `VideoGame`
- one three-level `BreadcrumbList`
...
- Existing titles, descriptions, canonicals, Open Graph and Twitter metadata are unchanged.
...
The structured-data workflow now regenerates both retro pages and canonical game wrappers in a temporary workspace, validates the resulting JSON-LD, checks generator scope and requires committed outputs to remain current.
