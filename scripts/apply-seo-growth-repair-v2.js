...
function ensureSocialMeta(html) {
  ...
  for (const [matcher, tag] of tags) {
    html = replaceOrInsertHeadTag(html, matcher, tag, /<meta\b(?=[^>]*\bproperty=["']og:title["'])[^>]*>/i);
  }
  return html;
}
function repairCategorySocialCards() {
  const roots = ["genres", "publishers", "developers", "collections", "platforms"]
...
function validate() {
  ...
  if (fs.existsSync(sample)) {
    ...
    if (!html.includes(`property="og:image" content="${SHARE_IMAGE}"`) || !html.includes(`name="twitter:image" content="${SHARE_IMAGE}"`)) {
