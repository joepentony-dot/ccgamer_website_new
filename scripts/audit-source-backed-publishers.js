...
// Completion gate: every publisher route currently generated by CCG must have
// exactly one researched, source-backed profile. This makes future publisher
// additions fail CI until their history/evidence is added as well.
const missingCurrentProfiles = [...publisherArchiveSlugs]
  .filter((slug) => !profileBySlug.has(slug))
  .sort();
const currentUnsourcedProfiles = [...publisherArchiveSlugs]
  .filter((slug) => {
    const profile = profileBySlug.get(slug);
    if (!profile) return false;
    return !(Array.isArray(profile.facts) && profile.facts.length > 0 && Array.isArray(profile.sources) && profile.sources.length > 0);
  })
  .sort();

if (missingCurrentProfiles.length) {
  failures.push(`Current publisher routes without researched profiles (${missingCurrentProfiles.length}): ${missingCurrentProfiles.join(", ")}`);
}
if (currentUnsourcedProfiles.length) {
  failures.push(`Current publisher routes without source-backed evidence (${currentUnsourcedProfiles.length}): ${currentUnsourcedProfiles.join(", ")}`);
}
...
console.log(`- ${publisherArchiveSlugs.size} current publisher routes have researched profiles`);
console.log(`- ${publisherArchiveSlugs.size} current publisher routes have source-backed evidence`);
console.log(`- ${sourceBackedCount} source-backed publisher profile(s) validated including dormant research records`);
...
console.log("- Future publisher routes cannot pass this audit without their own source-backed profile");
