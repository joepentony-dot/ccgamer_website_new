...
function renderAudit(plan, metadataChanged, genreResult) {
  ...
  const lines = [
    ...
    `- Genre fallback ordering: **preserved by the existing genre publishing contract**`,
...
function run(options = {}) {
  ...
  // Preserve the established genre fallback batch exactly as generated by
  // prepare-seo-genre-links.js. Opportunity data must not reorder those cards,
  // because validate-seo-genre-links.js treats source-order fallback links as a
  // deterministic publishing contract.
  const genreResult = { checked: 0, changed: 0, supported: new Set() };
...
function main(argv = process.argv.slice(2)) {
  ...
  console.log("[seo-opportunities] Genre fallback ordering preserved by existing publishing contract.");
