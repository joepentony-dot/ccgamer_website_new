...
ORIGINS = {"https://www.cheekycommodoregamer.co.uk", "https://cheekycommodoregamer.co.uk"}
...
def html_paths() -> list[Path]:
    ...
    for path in sorted(ROOT.rglob("*.html")):
        name = rel(path)
        if name in PROTECTED or name.startswith(EXCLUDED):
            continue
...
def target_file(value: str) -> Path | None:
    route = path_only(value)
    if route is None:
        return None
    if route == "/":
...
def scan() -> dict[str, Any]:
    ...
    for item in pages:
        ...
        if item["meta_target"] and item["script_target"] and path_only(item["meta_target"]) != path_only(item["script_target"]):
            findings["targets"].append({"path": item["path"], "meta": item["meta_target"], "script": item["script_target"]})
        target = item["target"]
        if not target:
            continue
        route = path_only(target)
        if route is None:
        ...
        if item["canonical"] and path_only(item["canonical"]) != route:
            findings["canonicals"].append({"path": item["path"], "canonical": item["canonical"], "target": target})
    source_templates = 0
    source_analytics = []
