...
- Native Mouse Wheel's first run failed only on unrelated Quiz first-response timing (280.3 ms) after every other audited page passed; one unchanged retry was started. No #2342 review finding was open at the documentation checkpoint.
- This documentation update creates a new exact head. Do not merge #2342 until that final head is still based on current main / 0 behind, mergeable, every required exact-head workflow is green, and no material review finding remains. After #2342 merges, reconcile #2339 onto the repaired main and fully requalify it before any CI-policy merge.
