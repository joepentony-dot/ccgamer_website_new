...
- Root cause: `decorate()` reserves mandatory rooms with `dedicatedHazardReserved`, but when the Sigil chamber consumes the only usable optional annex, the hidden trader previously fell back to `rooms[rooms.length-1]`. That fallback could claim a reserved mandatory room; `installDedicatedHazardRooms()` then treated it as busy and could produce zero dedicated hazards.
- Repair vehicle: draft PR #2342 / `codex/dungeon-hazard-reservation-trader-fix-20260925`, created directly from main `def0feab8ea92bfd271dd31d5d99b270af522239`. Runtime commit `9b4bb5d3d1065c61f8cf295c9552be5de384dd54` changes only the unsafe trader fallback to a non-reserved `featureRooms` candidate (or no fallback trader); test commit `73fd989a0541eac78a89074f3a9df09bc9127888` extends the R56 blocker contract to forbid the old fallback.
- Preliminary exact-head evidence on `73fd989a0541eac78a89074f3a9df09bc9127888`: canonical/Node contracts, Chromium shards 1, 2, 4, 5 and 6, Public Code Cache Version and the itch.io package passed. Shard 1 is the direct reproduction gate that failed on #2339, so its pass confirms this candidate closes that observed no-hazard seed. Shard 6 also passed the unchanged mobile natural-trap coverage that timed out on unrelated emulation PR #2341.
- Native Mouse Wheel's first run failed only on unrelated Quiz first-response timing (280.3 ms) after every other audited page passed; the unchanged retry passed on the documentation head.
- The documentation head `eb505ed6a3e50cc48a1b61e7da8842c7ae3bbcc8` then exposed a separate seed-dependent test-fixture collision in shard 5: `v10-41-stage8-encounter-chain-foundation.mjs` appended its synthetic secret door after generated doors, so `W.doorAt()` could select a real door first when coordinates collided. Test-only commit `4e0ef6e820d21a956d53bd5e128d9bc0ecd46c31` puts the synthetic fixture first; the canonical secret-door assertions are unchanged.
- This checkpoint update creates another exact head. Do not merge #2342 until that final head is still based on current main / 0 behind, mergeable, every required exact-head workflow is green, and no material review finding remains. After #2342 merges, reconcile #2339 onto the repaired main and fully requalify it before any CI-policy merge.
