...
## P0 FIRE lockout hotfix — 22 September 2026\n\nA live `V10.42 r50` Solo bug report captured firing working and then stopping during the same Floor 3 run. At failure the player retained 117 ammo, Tier 4 Tri-Pulse I, zero active player projectiles, gameplay focus and a hidden Inventory. Repeated fresh Space presses continued to move FIRE buffer state without consuming ammo or creating projectiles.\n\nThe priority remediation is isolated on `codex/dungeon-p0-fire-lockout-hotfix`, based directly on current live main, rather than waiting on unrelated R51 site/PWA work. Release identity advances to `V10.42 r51` / `20260922r51` so every ordered runtime module is fetched under the new canonical cache token.\n\nThe correction verifies valid quick taps even after keyup, counts only ammo consumption or a live player projectile as shot completion, makes R20 use the authoritative `bullets` collection, prevents a queued `fireBuffer1` from being reported as a successful direct repair, and applies the same actual-shot rule to Inventory recovery. A dedicated Chromium regression deliberately stages a 700 ms buffer with no shot after a released quick tap and requires delayed recovery to produce a real shot.\n\nStatus: **P0 HOTFIX IMPLEMENTED / EXACT-HEAD QUALIFICATION REQUIRED BEFORE MERGE**.\n\n## Active V10.42 R50 combined blocker candidate — 22 September 2026
