...
The curated file currently covers 15 C64 games / 16 UTA archive IDs. The exact-title regional/re-release approvals are Beyond the Forbidden Forest [1242], Blood 'n Guts [2809], Chopper [1036], Donald Duck's Playground [1240], Indiana Jones in the Lost Kingdom [4218], Karateka [2866], Mr Robot and his Robot Factory [3211], Raid on Bungeling Bay [5360], Raid Over Moscow [529], The Goonies [841] and [6951], They Stole a Million [12597], and Zorro [717]. Three independently verified semantic title aliases are also scoped by both game slug and exact archive ID: Aussie Games / Australian Games [20824], He-Man and the Masters of the Universe: The Movie / Masters of the Universe: The Movie [1319], and V: The Computer Game / V [2651]. Evidence URLs and concise verification notes are retained alongside each curated decision. These aliases do not become global substitutions. Entries whose title relationship is unsafe or whose identity is not independently established remain excluded; examples include Wizard/Wizard's Lair, Savage Pond/Savage, and Murder on the Mississippi/Murder.

Regression coverage requires a curated exact ID to remain scoped to its named game slug, rejects a deliberately curated prefix-only Karateka Championship candidate, and leaves an unapproved exact-title publisher mismatch in manual review. A second no-title-candidate pass also found safe normalisation gaps for word-boundary-only variants (`Dragonninja`/`Dragon Ninja`, `New Zealand`/`Newzealand`, `Nightbreed`/`Night Breed`) plus UTA's comma-article subtitle notation (`Train, The- Escape to Normandy`). The matcher now treats spacing-only title forms as exact-equivalent while retaining publisher/re-release and year evidence, and rewrites only the explicit raw `X, The- subtitle` form to canonical leading-article order before normalisation. Regression cases cover Bad Dudes vs Dragon Ninja, The New Zealand Story, Nightbreed, HeroQuest, High Noon, Micro Mouse Goes Debugging and The Train. The HeroQuest, High Noon and Micro Mouse cases are automatic publisher/year-backed spacing recoveries rather than curated overrides. Exact-head qualification exposed one additional publisher-normalisation gap: UTA records Micro Mouse under `M.C. Lothlorien` while CCG records `MC Lothlorien`. Publisher normalization now compacts dotted single-letter initialisms before applying label aliases, and regression coverage requires the two forms to compare equal. The continued residual sweep then found Stormbringer / `Storm Bringer` (Mastertronic Added Dimension, 1987) and Switchblade / `Switch Blade` (Gremlin Graphics (GBH), 1992). The spacing-equivalence rule covers both titles; archive-side publisher matching now decomposes parenthetical/composite UTA publisher credits using the same component-aware normalization as CCG game credits, allowing the Switch Blade release to match the existing Gremlin Graphics + GBH evidence without adding a broad alias. Exact-head regression then showed that the accepted Switch Blade release was being labelled as an original publisher release because the composite archive credit matched both Gremlin Graphics and GBH. Role selection now prefers a release component that is explicitly re-release-only in CCG data; therefore Gremlin Graphics (GBH) is retained as the same verified tape but presented as a re-release. A live UTA completeness check then exposed a parser-format gap: the current archive advertises 2,900 C64 releases while the last generated audit parsed 2,890, and UTA includes decade-unknown labels such as `199x` that the parser previously rejected. The parser now accepts generic `19xx`/`198x`/`199x`-style decade-unknown labels as year-neutral and fails closed when any release-looking `_[archive-id]` directory cannot be parsed, preventing future silent omissions. #2275 subsequently completed exact-head qualification and merged as `b40233c4d4debd6c50223582ff0a54eb9e5890ba`. Reliable Games Publishing run `35813642070` completed successfully and generated-output PR #2278 merged as `16a30183b40d3698564458401d309f385261e2f6`. The authoritative post-#2278 audit is 395/559 matched, 23 unmatched manual-review games, 141 unmatched no-title-candidate games, 164 unmatched total, and 2,892 retained UTA release directories.


## games.json residual reconciliation — 23 September 2026

Branch `codex/uta-games-json-residual-reconciliation-20260923` investigates the 164 residual unmatched C64 pages against their original `games/games.json` title, sort title, year, publisher, re-release and conversion identity before accepting any additional UTA relationship. The starting authoritative baseline is the post-#2278 audit: 559 C64 games, 395 matched, 23 unmatched manual-review games, 141 unmatched no-title-candidate games, and 2,892 parsed UTA release directories.

The completed residual classification is recorded in `data/uta-residual-review-2026-09-23.json`. Every one of the 164 baseline unmatched slugs appears exactly once. The current disposition is 17 verified additional tape pages, 4 catalogue-metadata corrections, 11 different-C64-version/game collisions, and 132 records with no verified compatible UTA directory. The last category means no compatible UTA directory was verified for the exact catalogue identity; it does not imply that no commercial cassette ever existed. The review retains reason subtypes for known disk-only, cartridge-only, modern non-tape and commercial-cassette-but-absent-from-UTA cases.

The comparison identified 17 additional same-game pages covering 20 cassette releases that are safe to expose after qualification: Action Biker [6254], Barbarian II: The Dungeon of Drax [5918], Boulder Dash [411], By Fair Means or Foul / Pro Boxing Simulator [2952], Dragon's Lair II: Escape from Singe's Castle [395] and [5078], Give My Regards to Broad Street [8446], Flunky / Mad Flunky [5141], International Soccer [3215], Leader Board [6678], Live and Let Die [2954], Nebulus [6721], Nightbreed: The Action Game [1544], Ninja [2849], Spy vs Spy II: The Island Caper [3935] plus its independently matched Hi-Tec release [1987], Strip Poker: A Sizzling Game of Chance [7489] and [513], Super Pipeline / Pipeline [6490], and The Sword of Fargoal [7724]. Where UTA uses a materially different name, the alias is scoped to the exact game slug and archive ID. The curated evidence file now contains 32 games / 35 curated IDs in total; the Hi-Tec Spy vs Spy II release remains automatic because the existing `games.json` re-release evidence should match it once duplicate archive IDs are preserved.

The same audit found a parser completeness defect independent of title matching: UTA archive IDs are not globally unique. UTA uses [1987] for both Solomon's Key / ERBE and Spy vs Spy II: The Island Caper / HiTEC. The parser previously de-duplicated globally by bare archive ID and could silently discard the later directory. Release de-duplication is now keyed by the UTA directory URL (with a composite fallback for accepted output), and regression coverage requires two distinct directories that share an archive ID to survive while duplicate copies of the same href remain collapsed.

The original `games.json` comparison also prevented false positives. After Burner is the separate 1989 Mindscape US conversion, not the earlier Activision/Hit Squad conversion represented by UTA. Double Dragon is the separate 1991 Ocean cartridge conversion, not the Melbourne House / Mastertronic Plus / Tronix tape conversion. Crossfire, Popeye, Kamikaze, Murder on the Mississippi, Savage Pond, Teenage Mutant Hero Turtles: The Coin-Op and Wizard likewise remain excluded where the UTA candidate is another game or another distinct conversion. Blockbusters is also treated as a version/game collision rather than attaching the 1984 Macsen Software release to the 1988 TV Games catalogue record.

Four catalogue records are intentionally left unchanged on this branch because the source record itself needs repair before a tape relationship is exposed. B.C. II: Grog's Revenge currently carries incompatible publisher/credit lineage instead of the documented Sierra On-Line / US Gold / Power House history; Nemesis is recorded as 1986 with no Hit Squad re-release although the C64 release is documented as 1987 with a 1990 Hit Squad cassette; Smash T.V. is recorded as 1992 / Codemasters / Allister Brimble although the C64 conversion is documented as a 1991 Ocean / Probe release programmed by Nick Jones with graphics by Lee Ames and music by Jeroen Tel; and the combined Kane & Kane 2 record needs splitting/reconciliation before Kane [7916] can be attached without misrepresenting Kane 2. Ninja is safe to map to Entertainment USA [2849], but its structured publisher field remains a secondary catalogue-cleanup item.

Ninja Massacre [10405] remains a medium-confidence exclusion: the UTA EDOS candidate is title-compatible, but the audit did not establish independent C64-specific evidence for an EDOS re-release. It remains visible in the residual review rather than being silently discarded.

The curated override file is an explicit trigger for both Phase 6B Reliable Games Editor Publishing and Reliable Games Publishing. Future evidence-only UTA curation changes therefore invoke validation and, after merge, the authoritative full-catalogue live scan without relying on another file changing at the same time. If the 17 verified new pages materialise as expected, the provisional post-refresh coverage is 412/559 matched and 147 unmatched; those figures remain provisional until the authoritative live UTA rescan runs. This candidate does not modify `games/games.json`.
