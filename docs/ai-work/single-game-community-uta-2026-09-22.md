...
The comparison identified 13 additional same-game cassette relationships that are safe to curate without changing the identity of the CCG page: Action Biker [6254], Barbarian II: The Dungeon of Drax [5918], Boulder Dash [411], Dragon's Lair II: Escape from Singe's Castle [395], International Soccer [3215], Leader Board [6678], Live and Let Die [2954], Nebulus [6721], Nightbreed: The Action Game [1544], Ninja [2849], Spy vs Spy II: The Island Caper [3935] plus its independently matched Hi-Tec release [1987], Strip Poker: A Sizzling Game of Chance [7489] and [513], and The Sword of Fargoal [7724]. Where UTA uses a materially different name, the alias is scoped to the exact game slug and archive ID. The curated evidence file therefore grows from 15 games / 16 IDs to 28 games / 30 curated IDs; the Hi-Tec Spy vs Spy II release is intentionally not curated because the existing `games.json` re-release evidence should match it automatically once the parser preserves it.
...
The original `games.json` comparison also prevented several false positives. After Burner is the separate 1989 Mindscape US conversion, not the earlier Activision/Hit Squad conversion represented by UTA. Double Dragon is the separate 1991 Ocean cartridge conversion, not the Melbourne House / Mastertronic Plus / Tronix tape conversion. Crossfire, Popeye, Kamikaze, Murder on the Mississippi, Savage Pond, Teenage Mutant Hero Turtles: The Coin-Op and Wizard similarly retain exclusion where the UTA candidate is another game or another distinct conversion.

Three source records are intentionally left unchanged on this branch because the problem is the game metadata itself rather than UTA matching: B.C. II: Grog's Revenge currently carries Software Projects / Charles Benton data from the predecessor lineage instead of the documented Sierra On-Line / US Gold / Power House release history; Nemesis is recorded as 1986 with no Hit Squad re-release although the C64 release is documented as 1987 with a 1990 Hit Squad cassette; and Smash T.V. is recorded as 1992 / Codemasters / Allister Brimble although the C64 conversion is documented as a 1991 Ocean / Probe release programmed by Nick Jones with graphics by Lee Ames and music by Jeroen Tel. Those require a separate source-data repair rather than a curated tape override.

Ninja Massacre [10405] remains excluded: the UTA EDOS candidate is title-compatible, but the audit did not establish independent C64-specific evidence for an EDOS re-release. Other lookalikes remain excluded unless equivalent source evidence is found.

The curated override file is now an explicit trigger for both Phase 6B Reliable Games Editor Publishing and Reliable Games Publishing. Future evidence-only UTA curation changes therefore invoke validation and, after merge, the authoritative full-catalogue live scan without relying on another file changing at the same time. This candidate does not modify `games/games.json`.
