...
## P0 Dungeon Carnage FIRE lockout hotfix — 22 September 2026\n\n- Hands-on `V10.42 r50` evidence reproduced a game-breaking Solo state where Space continued to generate attack intent with 117 ammo, a valid Tier 4 weapon, zero active player projectiles, visible/focused gameplay and no blocking Inventory, yet no shot was produced.\n- The priority fix is isolated from the broader R51 integration work on branch `codex/dungeon-p0-fire-lockout-hotfix`, based directly on current live `main` (`ea294bcc1ca4f3038e5360bff8f68ef5bf62d2f3`).\n- The hotfix advances the game to `V10.42 r51` / `20260922r51` solely so browsers cannot retain the broken r50 FIRE modules under stale cache URLs.\n- Fresh FIRE taps remain verifiable after keyup; queued buffer/cooldown movement is not accepted as a completed shot; R20 requires real ammo consumption or an authoritative live player projectile; Inventory recovery uses the same rule.\n- A Chromium regression stages the reproduced false-positive shape: a quick tap is released before delayed verification, a 700 ms FIRE buffer is present, the normal immediate shot is blocked, and the recovery must still produce a genuine shot.\n- This hotfix must remain unmerged until its exact head is fully qualified and merge is explicitly authorised.\n\n## Temporary CCG play-games maintenance — 22 September 2026
