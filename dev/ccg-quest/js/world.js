window.CCGWorld=(()=>{
  ...
  const THEMES={
    ...
    WARP_GALLERY:{name:"Warp Gallery",floor:"#111022",alt:"#181330",wall:"#453aa0",hi:"#7f70eb",accent:"#b978ff",message:"Purple energy crawls around the arches. There is an escape route somewhere nearby.",motif:"arches"},
  ...
  function createRooms(node,map,rooms,r){
    if(!node.left&&!node.right){
      ...
      rooms.push(room);node.roomId=room.id;carveRoom(map,room);return room.id;
    }
  ...
  function connectTree(node,map,rooms,edges,r){
    ...
    const ca=centre(rooms[a]),cb=centre(rooms[b]),path=carvePath(map,ca,cb,r);
    edges.push({a,b,path});
    node.roomId=r()<.5?a:b;
    return node.roomId;
  }
  ...
  function attachBonusRoom(map,source,index,rooms){
    const cx=Math.floor(source.x+source.w/2),cy=Math.floor(source.y+source.h/2);
    const opts=[
  ...
  function createHostState(w){
    ...
    const kinds=["scout","scout","ambusher","hunter","scout","guard","ghost","scout","ambusher","hunter","scout","guard","ghost","ambusher","scout","hunter","scout","ambusher"];
    let ei=0;
    for(const room of enemyRooms.slice(0,Math.min(30,enemyRooms.length))){
      const count=room.w*room.h>70&&w.random()<.38?2:1;
      for(let n=0;n<count&&ei<34;n++,ei++){
        const p=pickInRoom(w,room,used),kind=kinds[ei%kinds.length],hp=kind==="hunter"?5:kind==="guard"?4:kind==="ambusher"?3:kind==="ghost"?2:2;
        enemies.push({id:`e${ei}`,...p,kind,hp,maxHp:hp,alive:true,...aiFields(w)});
      }
    }
    const eliteRooms=[...openRooms].sort((a,b)=>b.depth-a.depth);
    C.followerElites.forEach((f,i)=>{const room=eliteRooms[(i*4+2)%eliteRooms.length]||openRooms[i%openRooms.length],p=pickInRoom(w,room,used);enemies.push({id:`f${i}`,...p,kind:f.kind,hp:f.hp,maxHp:f.hp,alive:true,follower:f,...aiFields(w)})});
    ...
    const cycle=["health","ammo","game","credits","torch","armour","potion","weapon","rapid","health","ammo","torch"];
    ...
    const doors=w.doorSpecs.map(d=>({id:d.id,x:d.x,y:d.y,roomId:d.roomId,locked:true,type:"bronze"}));
    const bronzeCount=Math.max(doors.length*2+4,12);for(let i=0;i<bronzeCount;i++){const p=pick(w,used,7,false);items.push({id:`bronze${i}`,...p,kind:"bronze",active:true})}
    ...
    while(ci<C.dungeon.chestCount&&deepOpen.length){const r=deepOpen[ci%deepOpen.length],p=pickInRoom(w,r,used);chests.push({id:`chest${ci}`,x:p.x,y:p.y,locked:ci%3!==0,active:true,reward:chestRewards[ci%chestRewards.length],roomId:r.id});ci++}
