...
| LS-SOLO-004 | OPEN | Runtime ownership repair activity is not yet bounded for stable Solo play | R30 owner seal can repair `movePlayer` every 16 ms and resets movement cooldowns on repair. R30 global movement guard can force-restore update/movement/damage ownership. Mode runtime can reinstall mode-owned gates and shared frame boundary. | Stable Solo soak records no unexplained ownership churn/reassertion growth during ordinary play; any repair is event-driven and attributable to a real ownership transition/fault. |
| LS-SOLO-005 | OPEN | Canonical load-safety gate is intermittently failing on the instrumentation candidate | The first Load Safety execution on `a02e1943...` failed in the canonical Chromium suite while the other visible gates passed. A rerun was requested before architectural changes. | Canonical load-safety rerun passes consistently, or a deterministic failure is isolated and fixed without weakening the tests. |
