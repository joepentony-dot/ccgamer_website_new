...
- **Memory Pad visibility/framing:** the complete Memory Pad room remains visible while the unsolved puzzle is active; Solo camera framing centres the full room and can zoom out to fit it. Mobile detection now uses the real viewport/coarse pointer instead of the sidebar-narrowed canvas.
...
- Latest verified Dungeon runtime merge checkpoint is #2222: exact qualified head `d27d714f5d15757e360ce27b49b4d185e379e56d`, merged as `cbbf9a97eb1f83c21eea3a519fd707af28be22bd`. It is based directly on #2220 (`f2d5332ceca250861e79185c943b8098c001894a`), the current Level 2 floor-simulation/trap-cycle liveness foundation. #2222 advances build/cache to `V10.42 r46` / `20260921r46`, adds bounded final visual maximisation, and makes a wrong Level 3 E/N/W/S torch input spawn exactly one monster.
- #2222's visual layer consumes established biome/room metadata and wraps existing render/trap presentation owners; it adds no perpetual polling/render loop and claims no simulation, collision, combat, progression, save or economy ownership. Severe-performance and reduced-motion fallbacks are retained.
- Exact #2222 qualification passed every triggered workflow. The first Load Safety shard-1 attempt completed the sustained Solo soak, live combat endurance and the five-minute mobile FIRE soak (15 verified attacks over 305,028 ms) before an unchanged post-soak `#resume-btn` locator timeout. One unchanged failed-job retry on the same head passed canonical/Node and all six Chromium shards; no runtime, assertion or timeout was weakened.
...
1. **Startup on current post-#2222 main** — loading must transition directly to the final V10.42 menu with no compact/intermediate menu flash and no loader → page → loader pulse; the stale-browser update path must retain the current Update Available label through V10.42 identity restamps.
...
4. **Mobile natural traps after #2201** — on the deployed mobile build, cross naturally generated active spike/fire/shock traps and confirm each real active contact removes one HEALTH while armour remains unchanged.
5. **Mobile landing/playfield** — confirm the full-width landing choices and 1.6x Solo/Tutorial camera remain usable on the deployed mobile build.
6. **r46 final visual polish** — confirm biome/room/trap presentation is performant and readable in real play.
7. **Level 3 directional torch** — confirm each incorrect E/N/W/S guess spawns exactly one monster; the separate flashing-floor memory puzzle must remain unchanged.
...
| NPC/content and merchant integration | PRESENT | Existing R7–R16 layers provide room objectives, breakables, encounter direction/progression, combat bridging, NPC data and environment presentation; Stage 7 #2140 completes the named NPC/merchant integration with contextual dialogue and optional quest/service hooks while preserving the established shop/economy owner. |
...
11. **Mobile playfield camera — REPOSITORY-COMPLETE THROUGH #2205 / DEPLOYED MANUAL RETEST REMAINS.** #2203 introduced a 1.3x mobile Solo/Tutorial camera; after hands-on feedback that it remained too small, #2205 increased the isolated mobile camera to 1.6x for canvas widths of 900px or narrower. Desktop and local 2P remain 1x. Exact-head qualification passed all nine triggered workflows, including the full Lost Sizzler Load Safety matrix and all six Chromium shards.
12. **Final game polish / visual maximisation — REPOSITORY-COMPLETE THROUGH #2222 / HANDS-ON REQUIRED.** r46 visual polish and the exact one-monster Level 3 directional-torch penalty are merged and fully qualified.
13. **Final qualification — EXACT-HEAD AUTOMATION PASS / HANDS-ON REQUIRED.** Repository coding is complete unless a hands-on gate exposes a new reproducible defect. Complete current startup, sustained Solo, Defect 5 Flask, mobile natural-trap damage, mobile landing/playfield and r46 visual/puzzle acceptance before external itch.io publication.
