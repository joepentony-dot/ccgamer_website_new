...
assert.equal(buildMeta,BUILD,"the blocking page identity must already be the current r33 build before any runtime restamp");
assert.equal(cacheMeta,CACHE,"the cache guard must read the r33 token on its first execution");
...
assert.match(bootstrap,/const BUILD="V10\.42 r33";/,"ordered bootstrap build identity changed unexpectedly");
assert.match(bootstrap,/const CACHE="20260918r33";/,"ordered bootstrap cache identity changed unexpectedly");
...
for(const asset of localAssets)assert.equal(asset.token,CACHE,`${asset.url} is not pinned to the current r33 cache token`);
assert.match(html,/game-local-runtime\.js\?v=20260918r33/,"the extracted current local runtime must not remain under the obsolete September 10 cache key");
