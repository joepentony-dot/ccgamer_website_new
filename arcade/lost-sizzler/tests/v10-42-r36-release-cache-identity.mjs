...
assert.equal(buildMeta,BUILD,"the blocking page identity must already be the current r54 build before any runtime restamp");
assert.equal(cacheMeta,CACHE,"the cache guard must read the r54 token on its first execution");
...
assert.match(bootstrap,/const BUILD="V10\.42 r54";/,"ordered bootstrap build identity changed unexpectedly");
assert.match(bootstrap,/const CACHE="20260924r54-critical3";/,"ordered bootstrap cache identity changed unexpectedly");
...
for(const asset of localAssets)assert.equal(asset.token,CACHE,`${asset.url} is not pinned to the current r54 cache token`);
assert.match(html,/game-local-runtime\.js\?v=20260924r54-critical3/,"the extracted current local runtime must not remain under the obsolete September 10 cache key");
assert.match(html,/game-main\.js\?v=20260924r54-critical3/,"the current input/frame owner must not remain under the obsolete September 10 cache key");
assert.match(html,/v10-41-cache-guard\.js\?v=20260924r54-critical3/,"the cache guard itself must be fetched under the current release token");
...
console.log("Dungeon Carnage r54 blocking release/cache identity contract passed.");
