...
const BUILD="V10.42 r41";
const CACHE="20260921r41";
...
assert.equal(buildMeta,BUILD,"the blocking page identity must already be the current r41 build before any runtime restamp");
...
for(const asset of localAssets)assert.equal(asset.token,CACHE,`${asset.url} is not pinned to the current r41 cache token`);
assert.match(html,/game-local-runtime\.js\?v=20260921r41/,"the extracted current local runtime must not remain under the obsolete September 10 cache key");
