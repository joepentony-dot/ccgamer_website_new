...
assert.match(bootstrap,/const BUILD="V10\.42 r57";/,"ordered bootstrap build identity changed unexpectedly");
assert.match(bootstrap,/const CACHE="20260925r58";/,"ordered bootstrap cache identity changed unexpectedly");
...
for(const asset of localAssets)assert.equal(asset.token,CACHE,`${asset.url} is not pinned to the current r57 cache token`);
assert.match(html,/game-local-runtime\.js\?v=20260925r58/,"the extracted current local runtime must not remain under the obsolete September 10 cache key");
assert.match(html,/game-main\.js\?v=20260925r58/,"the current input/frame owner must not remain under the obsolete September 10 cache key");
assert.match(html,/v10-41-cache-guard\.js\?v=20260925r58/,"the cache guard itself must be fetched under the current release token");
...
console.log("Dungeon Carnage r57 blocking release/cache identity contract passed.");
