...
assert.equal(buildMeta,BUILD,"the blocking page identity must already be the current r48 build before any runtime restamp");
assert.equal(cacheMeta,CACHE,"the cache guard must read the r48 token on its first execution");
...
assert.match(bootstrap,/const BUILD="V10\.42 r48";/,"ordered bootstrap build identity changed unexpectedly");
...
for(const asset of localAssets)assert.equal(asset.token,CACHE,`${asset.url} is not pinned to the current r48 cache token`);
...
assert.match(bootstrap,/v10-42-stage6-zone-gameplay\.js/,"r48 must load the Stage 6 zone gameplay owner through the ordered bootstrap");

console.log("Dungeon Carnage r48 blocking release/cache identity contract passed.");
