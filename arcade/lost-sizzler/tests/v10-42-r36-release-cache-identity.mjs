...
const BUILD="V10.42 r40";
const CACHE="20260921r40";
...
assert.equal(buildMeta,BUILD,"the blocking page identity must already be the current r40 build before any runtime restamp");
...
for(const asset of localAssets)assert.equal(asset.token,CACHE,`${asset.url} is not pinned to the current r40 cache token`);
assert.match(html,/game-local-runtime\.js\?v=20260921r40/,"the extracted current local runtime must not remain under the obsolete September 10 cache key");
assert.match(html,/game-main\.js\?v=20260921r40/,"the current input/frame owner must not remain under the obsolete September 10 cache key");
assert.match(html,/v10-41-cache-guard\.js\?v=20260921r40/,"the cache guard itself must be fetched under the current release token");
