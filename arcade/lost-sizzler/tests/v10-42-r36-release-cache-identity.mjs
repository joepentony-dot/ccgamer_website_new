...
const BUILD="V10.42 r48";
const CACHE="20260922r48";
...
assert.equal(buildMeta,BUILD,"the blocking page identity must already be the current r47 build before any runtime restamp");
assert.equal(cacheMeta,CACHE,"the cache guard must read the r47 token on its first execution");
...
assert.match(bootstrap,/const BUILD="V10\.42 r47";/,"ordered bootstrap build identity changed unexpectedly");
assert.match(bootstrap,/const CACHE="20260922r48";/,"ordered bootstrap cache identity changed unexpectedly");
...
for(const asset of localAssets)assert.equal(asset.token,CACHE,`${asset.url} is not pinned to the current r47 cache token`);
assert.match(html,/game-local-runtime\.js\?v=20260922r48/,"the extracted current local runtime must not remain under the obsolete September 10 cache key");
assert.match(html,/game-main\.js\?v=20260922r48/,"the current input/frame owner must not remain under the obsolete September 10 cache key");
assert.match(html,/v10-41-cache-guard\.js\?v=20260922r48/,"the cache guard itself must be fetched under the current release token");

assert.match(bootstrap,/v10-42-stage6-zone-gameplay\.js/,"r47 must load the Stage 6 zone gameplay owner through the ordered bootstrap");

console.log("Dungeon Carnage r47 blocking release/cache identity contract passed.");
