...
assert.match(mod,/document\.addEventListener\("click"[\s\S]*closest\("\[data-shop-buy\]"\)[\s\S]*purchaseDepth\+\+[\s\S]*fallbackFeedback\(id,false\)/,"direct shop-button owners must still create a bounded purchase-feedback window and re-evaluate authoritative blockers");
assert.match(mod,/function fallbackFeedback\(id,allowGeneric=true\)[\s\S]*if\(!allowGeneric\)return false/,"post-click blocker reconciliation must not invent a generic failure after a successful purchase");
