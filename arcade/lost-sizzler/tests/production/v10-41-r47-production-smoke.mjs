...
const checkedOutVersion=JSON.parse(fs.readFileSync(new URL("../../version.json",import.meta.url),"utf8"));
const expectedReleaseVersion=String(checkedOutVersion.releaseVersion||"");
const expectedBuild=String(checkedOutVersion.build||"");
const expectedCacheToken=String(checkedOutVersion.cacheToken||"");
assert.ok(expectedReleaseVersion&&expectedBuild&&expectedCacheToken,"checked-out version.json must define releaseVersion, build and cacheToken");
const syntheticStaleBuild="2026.09.10.0-production-smoke";
const versionUrl=new URL("/arcade/lost-sizzler/version.json",gameUrl).toString();
const checkedOutGameSource=fs.readFileSync(new URL("../../index.html",import.meta.url),"utf8");
assert.ok(checkedOutGameSource.includes(`ccg-lost-sizzler-build" content="${expectedBuild}`),"checked-out game source must agree with version.json build");
assert.ok(checkedOutGameSource.includes(`ccg-lost-sizzler-cache" content="${expectedCacheToken}`),"checked-out game source must agree with version.json cache token");
