...
// sprite re-encoding in the release-gate finish callback. The r21 guard must
...
assert.match(lake,/load\("js\/v10-41-horde-mode-safety\.js","data-ccg-v141-horde-mode-safety"\)/,"r21 safety entry must request the fresh Horde isolation guard through the shared release token");
assert.match(lake,/load\("js\/v10-41-multiplayer-no-pause\.js","data-ccg-v141-multiplayer-no-pause"\)/,"r21 safety entry must request multiplayer no-pause hardening through the shared release token");
...
assert.equal(manifest.build,"2026.08.25.21","Horde stability fixes must be published as build .21");
assert.equal(manifest.cacheToken,"20260825r21","Horde stability fixes must force the r21 cache shell");
assert.match(index,/ccg-lost-sizzler-build" content="2026\.08\.25\.21"/,"HTML build marker must match r21");
assert.match(index,/ccg-lost-sizzler-cache" content="20260825r21"/,"HTML cache marker must match r21");
