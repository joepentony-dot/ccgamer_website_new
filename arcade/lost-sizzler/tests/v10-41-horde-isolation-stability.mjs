...
assert.equal(manifest.build,"2026.08.26.30","Horde stability fixes must remain included in build .29");
assert.equal(manifest.cacheToken,"20260826r30","Horde stability fixes must use the current r29 cache shell");
assert.match(index,/ccg-lost-sizzler-build" content="2026\.08\.25\.29"/,"HTML build marker must match r29");
assert.match(index,/ccg-lost-sizzler-cache" content="20260826r30"/,"HTML cache marker must match r29");
assert.match(index,/v10-41-r28-special-mode-repair\.js\?v=20260826r30/,"canonical HTML must load the retained r28 special-mode repair under the current r29 cache generation");
