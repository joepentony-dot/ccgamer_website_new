...
// sprite re-encoding in the release-gate finish callback. The r22 guard must
...
assert.match(lake,/load\("js\/v10-41-horde-mode-safety\.js","data-ccg-v141-horde-mode-safety"\)/,"r22 safety entry must request the fresh Horde isolation guard through the shared release token");
assert.match(lake,/load\("js\/v10-41-multiplayer-no-pause\.js","data-ccg-v141-multiplayer-no-pause"\)/,"r22 safety entry must request multiplayer no-pause hardening through the shared release token");
...
assert.equal(manifest.build,"2026.08.25.22","Horde stability fixes must be published as build .22");
assert.equal(manifest.cacheToken,"20260825r22","Horde stability fixes must force the r22 cache shell");
assert.match(index,/ccg-lost-sizzler-build" content="2026\.08\.25\.22"/,"HTML build marker must match r22");
assert.match(index,/ccg-lost-sizzler-cache" content="20260825r22"/,"HTML cache marker must match r22");
