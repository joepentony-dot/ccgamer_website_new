...
// sprite re-encoding in the release-gate finish callback. The guard remains
// part of the r21 release and must never perform its own data URL conversion.
...
assert.match(lake,/v10-41-horde-mode-safety\.js\?v=20260825e/,"r21 safety entry must request the fresh Horde isolation guard");
assert.match(lake,/v10-41-multiplayer-no-pause\.js\?v=20260825e/,"r21 safety entry must request multiplayer no-pause hardening");
...
assert.equal(manifest.build,"2026.08.25.21","current Horde stability fixes must be published as build .21");
assert.equal(manifest.cacheToken,"20260825r21","current Horde stability fixes must force the r21 cache shell");
