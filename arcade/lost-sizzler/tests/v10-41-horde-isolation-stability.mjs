...
// sprite re-encoding in the release-gate finish callback. The r24 guard must
// retain the bypass and never perform its own data URL conversion.
...
assert.match(lake,/load\("js\/v10-41-horde-mode-safety\.js","data-ccg-v141-horde-mode-safety"\)/,"r24 safety entry must request the fresh Horde isolation guard through the shared release token");
assert.match(lake,/load\("js\/v10-41-multiplayer-no-pause\.js","data-ccg-v141-multiplayer-no-pause"\)/,"r24 safety entry must request multiplayer no-pause hardening through the shared release token");
...
assert.equal(manifest.build,"2026.08.25.24","Horde stability fixes must remain published in build .24");
assert.equal(manifest.cacheToken,"20260825r24","Horde stability fixes must use the r24 cache shell");
assert.match(index,/ccg-lost-sizzler-build" content="2026\.08\.25\.24"/,"HTML build marker must match r24");
assert.match(index,/ccg-lost-sizzler-cache" content="20260825r24"/,"HTML cache marker must match r24");
