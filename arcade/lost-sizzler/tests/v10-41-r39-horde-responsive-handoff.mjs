...
assert.match(responsive,/\.v102-game-area\{[\s\S]*?height:100%!important;[\s\S]*?min-height:0!important;/,"Horde game area must override the retained height:auto rule and fill its viewport row");
...
assert.match(responsive,/\.tactical-zone\{[\s\S]*?grid-template-rows:minmax\(0,1fr\) auto!important;/,"desktop Horde side column must use its height for radar plus the live player roster");
...
assert.match(responsive,/grid-template-rows:auto minmax\(0,1fr\) auto auto!important;height:100dvh!important;max-height:100dvh!important/,"tablet Horde must remain within one dynamic viewport rather than growing into a huge vertical canvas");
...
console.log("Lost Sizzler V10.41 r39 Horde responsive viewport and dedicated-handoff contracts passed.");
