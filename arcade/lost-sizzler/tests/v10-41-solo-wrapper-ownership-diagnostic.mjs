...
// Post/R56/R60 layer to be appended on the next maintenance cycle. R29 keeps
// its generic ancestry helper for compatibility but no longer owns damage.
...
assert.doesNotMatch(r29,/__ccgV141R29HordeFriendly|installHordeFriendlyFireGuard/,"R29 must no longer install retired Horde damage ownership");
...
assert.match(post,/originalChainHasMarker\(current,"__ccgV141SpyDamageBoundary"\)/,"post-playtest damage guard must not bury the legacy isolated Spy boundary while it remains downstream compatibility metadata");
...
assert.match(r60,/wrapped\.__ccgV141R60EnvironmentSeal=true;wrapped\.__ccgV141R56EnvironmentDamage=true;wrapped\.__ccgV141R29HordeFriendly=true/,"R60 must retain the historical R56/R29 compatibility markers until its own later cleanup stage");
...
const retainedLayers=postLayers+r56Layers+r60Layers;

assert.equal(postLayers,1,`post-playtest damage ownership multiplied to ${postLayers} layers`);
assert.equal(r29Layers,0,`retired R29 damage ownership unexpectedly returned with ${r29Layers} layers`);
...
console.log(`Solo damage ownership ceiling passed: retained Post/R56/R60 layers=${retainedLayers}, retired R29 layers=${r29Layers}, total synthetic depth ${initialDepth} -> ${finalDepth} across ${cycles} external-owner cycles.`);