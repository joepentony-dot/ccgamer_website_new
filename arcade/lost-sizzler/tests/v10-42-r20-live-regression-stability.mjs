...
assert.match(stallElapsed,/debtMs=Math\.min\(RAF_RECOVERY_MAX_DEBT_MS,carried\+Math\.max\(0,gap-NORMAL_FRAME_MS\)\)[\s\S]*recovery=\{debtMs,frames:0,lastFrameTimestamp:t\}/,"an active browser stall must record only bounded excess wall-clock time as recoverable simulation debt");
assert.match(stallElapsed,/desiredStep=Math\.min\(RAF_RECOVERY_MAX_STEP_MS,Math\.max\(NORMAL_FRAME_MS,wallStep\)\+debtBefore\)[\s\S]*repaid=Math\.max\(0,desiredStep-wallStep\)[\s\S]*recovery\.debtMs=Math\.max\(0,debtBefore-repaid\)/,"active stall debt must be repaid gradually instead of being injected as one large simulation frame");
