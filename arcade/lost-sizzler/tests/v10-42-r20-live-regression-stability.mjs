...
assert.match(play,/const damageAt=performance\.now\(\);p\.__ccgLastHurtAt=damageAt;p\.__ccgLastDamageAt=damageAt;p\.__ccgLastDamageSource=String\(source\|\|"enemy"\);p\.hitStunMs=/,"core damage handling must timestamp legitimate hit-stun ownership while retaining source-attributed diagnostic evidence");
