...
assert.match(play,/const damageAt=performance\.now\(\),damageSource=String\(source\|\|"enemy"\);p\.__ccgLastHurtAt=damageAt;p\.__ccgLastDamageAt=damageAt;p\.__ccgLastDamageSource=damageSource;[\s\S]*p\.hitStunMs=/,"core damage handling must timestamp legitimate hit-stun ownership while retaining source-attributed diagnostic evidence");
