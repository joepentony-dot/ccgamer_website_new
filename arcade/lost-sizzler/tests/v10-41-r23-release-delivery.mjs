...
assert.equal(version.build,"2026.08.26.30","current stability release must publish build 2026.08.26.30");
assert.equal(version.cacheToken,"20260826r30","current stability release must publish cache generation r30");
assert.match(index,/ccg-lost-sizzler-build" content="2026\.08\.26\.30"/,"HTML build metadata must match r29");
assert.match(index,/ccg-lost-sizzler-cache" content="20260826r30"/,"HTML cache metadata must match r29");
...
assert.match(index,/v10-41-r25-spy-speed-bounty-hotfix\.js\?v=20260826r30/,"the r25 Spy speed/bounty hotfix must remain directly loaded under the current r30 cache shell");
assert.match(index,/v10-41-r26-spy-enemy-stability\.js\?v=20260826r30/,"the r26 Spy/enemy stability layer must remain loaded before the later isolation layers");
assert.match(index,/v10-41-r27-spy-isolation\.js\?v=20260826r30/,"the r27 Spy isolation layer must remain loaded beneath r28/r29");
assert.match(index,/v10-41-r28-special-mode-repair\.js\?v=20260826r30/,"the retained r28 special-mode repair must load beneath r29");
assert.match(index,/v10-41-r29-runtime-repair\.js\?v=20260826r30/,"the r29 runtime repair must be the final canonical gameplay layer");
assert.ok(index.indexOf("v10-41-r25-spy-speed-bounty-hotfix.js?v=20260826r30")<index.indexOf("v10-41-r26-spy-enemy-stability.js?v=20260826r30"),"r26 must load after r25");
assert.ok(index.indexOf("v10-41-r26-spy-enemy-stability.js?v=20260826r30")<index.indexOf("v10-41-r27-spy-isolation.js?v=20260826r30"),"r27 must load after r26");
assert.ok(index.indexOf("v10-41-r27-spy-isolation.js?v=20260826r30")<index.indexOf("v10-41-r28-special-mode-repair.js?v=20260826r30"),"r28 must load after r27");
assert.ok(index.indexOf("v10-41-r28-special-mode-repair.js?v=20260826r30")<index.indexOf("v10-41-r29-runtime-repair.js?v=20260826r30"),"r29 must load after the retained r28 repair layer");
...
assert.match(network,/v10-41-live-join-presence\.js\?v=\$\{encodeURIComponent\(releaseRev\)\}/,"network live-join loader must inherit r30 from page metadata");
...
console.log("Lost Sizzler V10.41 r30 release-delivery contract passed.");
