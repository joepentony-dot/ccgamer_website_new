...
assert.equal(version.build,"2026.08.25.26","stability release must publish build 2026.08.25.26");
assert.equal(version.cacheToken,"20260825r26","stability release must publish cache generation r26");
assert.match(index,/ccg-lost-sizzler-build" content="2026\.08\.25\.26"/,"HTML build metadata must match r26");
assert.match(index,/ccg-lost-sizzler-cache" content="20260825r26"/,"HTML cache metadata must match r26");
assert.doesNotMatch(index,/20260825r(?:20|21|22|23|24|25)|2026\.08\.25\.(?:20|21|22|23|24|25)/,"canonical HTML must contain no superseded r20–r25 release token");
...
assert.match(index,/v10-41-r25-spy-speed-bounty-hotfix\.js\?v=20260825r26/,"the r25 Spy speed/bounty hotfix must remain directly loaded under the current r26 cache shell");
assert.match(index,/v10-41-r26-spy-enemy-stability\.js\?v=20260825r26/,"the r26 Spy/enemy stability layer must be directly loaded last by the canonical page");
assert.ok(index.indexOf("v10-41-r25-spy-speed-bounty-hotfix.js?v=20260825r26")<index.indexOf("v10-41-r26-spy-enemy-stability.js?v=20260825r26"),"r26 must load after r25");
...
assert.match(network,/v10-41-live-join-presence\.js\?v=\$\{encodeURIComponent\(releaseRev\)\}/,"network live-join loader must inherit r26 from page metadata");
...
console.log("Lost Sizzler V10.41 r26 release-delivery contract passed.");
