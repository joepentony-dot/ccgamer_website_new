...
assert.equal(version.build,"2026.08.25.28","current stability release must publish build 2026.08.25.28");
assert.equal(version.cacheToken,"20260825r28","current stability release must publish cache generation r28");
assert.match(index,/ccg-lost-sizzler-build" content="2026\.08\.25\.28"/,"HTML build metadata must match r28");
assert.match(index,/ccg-lost-sizzler-cache" content="20260825r28"/,"HTML cache metadata must match r28");
assert.doesNotMatch(index,/20260825r(?:20|21|22|23|24|25|26|27)|2026\.08\.25\.(?:20|21|22|23|24|25|26|27)/,"canonical HTML must contain no superseded r20–r27 release token");
...
assert.match(index,/v10-41-r25-spy-speed-bounty-hotfix\.js\?v=20260825r28/,"the r25 Spy speed/bounty hotfix must remain directly loaded under the current r28 cache shell");
assert.match(index,/v10-41-r26-spy-enemy-stability\.js\?v=20260825r28/,"the r26 Spy/enemy stability layer must remain loaded before the later isolation layers");
assert.match(index,/v10-41-r27-spy-isolation\.js\?v=20260825r28/,"the r27 Spy isolation layer must remain loaded beneath r28");
assert.match(index,/v10-41-r28-special-mode-repair\.js\?v=20260825r28/,"the r28 repair layer must be the final canonical gameplay layer");
assert.ok(index.indexOf("v10-41-r25-spy-speed-bounty-hotfix.js?v=20260825r28")<index.indexOf("v10-41-r26-spy-enemy-stability.js?v=20260825r28"),"r26 must load after r25");
assert.ok(index.indexOf("v10-41-r26-spy-enemy-stability.js?v=20260825r28")<index.indexOf("v10-41-r27-spy-isolation.js?v=20260825r28"),"r27 must load after r26");
assert.ok(index.indexOf("v10-41-r27-spy-isolation.js?v=20260825r28")<index.indexOf("v10-41-r28-special-mode-repair.js?v=20260825r28"),"r28 must load after r27");
...
assert.match(r28,/__CCG_LOST_SIZZLER_V141_R28_SPECIAL_MODE_REPAIR__/,"r28 final repair must retain its duplicate-install guard");
...
assert.match(network,/v10-41-live-join-presence\.js\?v=\$\{encodeURIComponent\(releaseRev\)\}/,"network live-join loader must inherit r28 from page metadata");
...
console.log("Lost Sizzler V10.41 r28 release-delivery contract passed.");
