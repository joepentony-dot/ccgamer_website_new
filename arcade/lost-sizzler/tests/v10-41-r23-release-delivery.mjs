...
assert.match(late,/load\("js\/v10-41-tutorial-action-finalizer\.js","data-ccg-v141-tutorial-action-finalizer"\)/,"the post-release tutorial action owner must be included in the r23 late runtime chain");
...
assert.match(network,/v10-41-live-join-presence\.js\?v=\$\{encodeURIComponent\(releaseRev\)\}/,"network live-join loader must inherit r23 from page metadata");
assert.match(tutorialFinal,/function installMove\(\)/,"r23 tutorial hardening must retain final movement ownership");
assert.match(tutorialFinal,/function installFire\(\)/,"r23 tutorial hardening must retain final sword ownership");
assert.match(tutorialFinal,/function installDash\(\)/,"r23 tutorial hardening must retain final dash ownership");

console.log("Lost Sizzler V10.41 r23 release-delivery contract passed.");
