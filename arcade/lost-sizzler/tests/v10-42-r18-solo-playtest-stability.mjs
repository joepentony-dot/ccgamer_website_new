...
assert.match(main,/function settlePauseAttackCadence\(reason="resume"\)[\s\S]*CCGLostSizzlerV142R18SoloPlaytestStability\?\.repairAttackLiveness\?\.\("pause-resume"\)[\s\S]*queueMicrotask\(settle\)[\s\S]*setTimeout\(settle,0\)/,"resume must retain a post-event repair path after competing input owners have run");
