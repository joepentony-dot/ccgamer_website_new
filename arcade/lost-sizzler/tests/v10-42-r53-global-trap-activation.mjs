...
assert.match(systems,/hazardReserveLarge=[\\s\\S]*w>=6&&room\\.h>=5[\\s\\S]*hazardReserveCompact=[\\s\\S]*w>=3&&room\\.h>=3[\\s\\S]*hazardReserveOptional=[\\s\\S]*room\\?\\.optional[\\s\\S]*w>=3&&room\\.h>=3[\\s\\S]*hazardReserveRooms=\\[[\\s\\S]*hazardReservePriority\\(hazardReserveLarge\\)[\\s\\S]*hazardReservePriority\\(hazardReserveCompact\\)[\\s\\S]*hazardReservePriority\\(hazardReserveOptional\\)/,"compact floors must layer large, compact and optional reservation tiers before allowing a mandatory dedicated hazard to disappear");
