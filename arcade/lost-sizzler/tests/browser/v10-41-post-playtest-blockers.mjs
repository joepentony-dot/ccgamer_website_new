  ...
  if(zeroServer){
    const retired=await page.evaluate(()=>{
      ...
      return{
        onlineMultiplayer:document.body.dataset.onlineMultiplayer||"",
        releaseModel:document.body.dataset.releaseModel||"",
        hordeEntryHidden:!button||Boolean(button.hidden||button.classList.contains("hidden")||style?.display==="none"),
    ...
    if(retired.hordeAriaHidden)assert.equal(retired.hordeAriaHidden,"true","a retained retired Horde control must remain hidden from accessibility navigation");
    ...
    // Historic Horde arena coverage is intentionally not executable after
    // retirement; the zero-server boundary above is the retained contract.
    await page.evaluate(()=>{
