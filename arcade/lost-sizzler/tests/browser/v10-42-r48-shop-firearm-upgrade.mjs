...
try{
  ...
  assert.equal(result.normalPriceBefore,1000);
  ...
  assert.equal(result.after1.normalPrice,1000,"firearm purchase must not advance the ordinary shop ladder");
  ...
  assert.equal(result.after2.normalPrice,1000);
  ...
  assert.equal(result.blocked.normalPrice,1000);
  ...
  assert.equal(result.floor2.normalPrice,1000);
  ...
  console.log("Dungeon Carnage r48 shop firearm upgrade browser contract passed.");
  await context.close();
}finally{
