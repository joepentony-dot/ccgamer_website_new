...
try{
  ...
  const shop=await page.evaluate(()=>{
    ...
    for(let i=0;i<3;i++){buyShopItem("ammo");prices.push(shopScorePrice(fake))}
    closeShop();
    return{prices,count:fake.scorePurchases,shadow:fake.__v142ScorePurchases};
  });
  assert.deepEqual(shop.prices,[1000,2000,4000,8000],"Repeated score purchases in one shop must progress 1,000 → 2,000 → 4,000 → 8,000.");
  assert.equal(shop.count,3,"Three successful score purchases must advance the shop counter three times.");
  assert.equal(shop.shadow,3,"V10.42 stability counter must stay synchronized with the shop counter.");
  ...
  const chest=await page.evaluate(()=>{
    ...
    openChest(p1,fake);return{active:fake.active,rewardScore:fake.rewardScore,rewardXp:fake.rewardXp};
  });
  ...
  console.log("Lost Sizzler V10.42 r1 combat, shop, dossier, chest and collectible stability browser regression passed.");
  await context.close();
}finally{
  await browser.close();
