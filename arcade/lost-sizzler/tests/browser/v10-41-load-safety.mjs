...
try{
  ...
  await page.evaluate(async()=>{
    ...
    localStorage.setItem("ccg-lost-sizzler:last-sanitised-cache","20260825r25");
  });
  ...
  async function loadAndAudit(iteration){
    ...
    assert.equal(audit.cacheToken,"20260825r26",`iteration ${iteration}: current cache token must be r26`);
    assert.equal(audit.storedToken,"20260825r26",`iteration ${iteration}: successful sanitation must record r26`);
  ...
  assert.equal(first.cacheGuard.needed,true,"first visit from r25 must perform r26 game-cache sanitation");
  ...
  assert.equal(second.cacheGuard.needed,false,"second r26 visit must not repeatedly purge caches");
  ...
  console.log("Lost Sizzler V10.41 r26 repeated Chromium load, r25-to-r26 cache sanitation and main-thread responsiveness checks passed.");
  await context.close();
}finally{
  await browser.close();
