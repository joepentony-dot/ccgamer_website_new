...
try{
  ...
  await page.evaluate(async()=>{
    ...
    localStorage.setItem("ccg-lost-sizzler:last-sanitised-cache","20260825r26");
  });
  ...
  async function loadAndAudit(iteration){
    ...
    assert.equal(audit.cacheToken,"20260825r27",`iteration ${iteration}: current cache token must be r27`);
    assert.equal(audit.storedToken,"20260825r27",`iteration ${iteration}: successful sanitation must record r27`);
  ...
  assert.equal(first.cacheGuard.needed,true,"first visit from r26 must perform r27 game-cache sanitation");
  ...
  assert.equal(second.cacheGuard.needed,false,"second r27 visit must not repeatedly purge caches");
  ...
  console.log("Lost Sizzler V10.41 r27 repeated Chromium load, r26-to-r27 cache sanitation and main-thread responsiveness checks passed.");
  await context.close();
}finally{
  await browser.close();
