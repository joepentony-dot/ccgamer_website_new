...
try{
  ...
  const traps=await page.evaluate(()=>{
    const api=window.CCGLostSizzlerV141R57DesktopPrepStability;api.bridgeR56();const environmentOwner=api.chainHas(window.hurtPlayer,"__ccgV141R56EnvironmentDamage");api.state.trapCycles.clear();p1.armor=0;p1.maxHealth=Math.max(12,p1.maxHealth);p1.health=12;p1.invuln=0;
    const fire={id:"r57-fire",kind:"fire",x:p1.x,y:p1.y,active:true,period:10000000,phase:0};host.traps=[fire];const start=p1.health;api.contactTick();const fire1=p1.health;fire.phase=6000000;api.contactTick();fire.phase=0;p1.invuln=0;api.contactTick();const fire2=p1.health;
    api.state.trapCycles.clear();const spike={id:"r57-spike",kind:"spike",x:p1.x,y:p1.y,active:true,period:10000000,phase:0};host.traps=[spike];p1.invuln=0;api.contactTick();const spike1=p1.health;
