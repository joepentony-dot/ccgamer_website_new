...
try{
  ...
  assert.equal(boot.build,"V10.42 r51");assert.equal(boot.cache,"20260923r51");assert.equal(boot.metaBuild,"V10.42 r51");assert.equal(boot.metaCache,"20260923r51");
  assert.ok(boot.ordered.length>=30,"r47 bootstrap must load the complete ordered V10.42 chain");
  assert.ok(boot.ordered.every(src=>new URL(src).searchParams.get("v")==="20260923r51"),"every ordered V10.42 module must use the r51 cache token");
  assert.ok(v142Requests.some(src=>src.includes("v10-42-projectile-lifecycle.js?v=20260923r51")),"expected r51 projectile lifecycle asset was not requested");
