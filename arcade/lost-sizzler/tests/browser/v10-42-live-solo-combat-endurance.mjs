...
try{
  ...
  assert.equal(boot.build,"V10.42 r48");assert.equal(boot.cache,"20260922r48");assert.equal(boot.metaBuild,"V10.42 r48");assert.equal(boot.metaCache,"20260922r48");
  assert.ok(boot.ordered.length>=30,"r47 bootstrap must load the complete ordered V10.42 chain");
  assert.ok(boot.ordered.every(src=>new URL(src).searchParams.get("v")==="20260922r48"),"every ordered V10.42 module must use the r46 cache token");
  assert.ok(v142Requests.some(src=>src.includes("v10-42-projectile-lifecycle.js?v=20260922r48")),"expected r47 projectile lifecycle asset was not requested");
