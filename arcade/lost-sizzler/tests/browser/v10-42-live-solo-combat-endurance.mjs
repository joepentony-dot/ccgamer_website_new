...
try{
  ...
  assert.equal(boot.failed,false,`r31 ordered bootstrap failed: ${boot.error}`);assert.equal(boot.ready,true,"r31 ordered bootstrap must complete");
  assert.equal(boot.build,"V10.42 r31");assert.equal(boot.cache,"20260917r31");assert.equal(boot.metaBuild,"V10.42 r31");assert.equal(boot.metaCache,"20260917r31");
  assert.ok(boot.ordered.length>=30,"r31 bootstrap must load the complete ordered V10.42 chain");
  assert.ok(boot.ordered.every(src=>new URL(src).searchParams.get("v")==="20260917r31"),"every ordered V10.42 module must use the r31 cache token");
  assert.ok(v142Requests.some(src=>src.includes("v10-42-projectile-lifecycle.js?v=20260917r31")),"expected r31 projectile lifecycle asset was not requested");
  ...
  console.log("Dungeon Carnage V10.42 r31 live Solo combat endurance regression passed.");
  await context.close();
}finally{
