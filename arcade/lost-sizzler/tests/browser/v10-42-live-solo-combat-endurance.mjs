...
try{
  ...
  assert.equal(boot.build,"V10.42 r33");assert.equal(boot.cache,"20260918r33");assert.equal(boot.metaBuild,"V10.42 r33");assert.equal(boot.metaCache,"20260918r33");
  assert.ok(boot.ordered.length>=30,"r32 bootstrap must load the complete ordered V10.42 chain");
  assert.ok(boot.ordered.every(src=>new URL(src).searchParams.get("v")==="20260918r33"),"every ordered V10.42 module must use the r32 cache token");
  assert.ok(v142Requests.some(src=>src.includes("v10-42-projectile-lifecycle.js?v=20260918r33")),"expected r32 projectile lifecycle asset was not requested");
  ...
  console.log("Dungeon Carnage V10.42 r33 live Solo combat endurance regression passed.");
  await context.close();
}finally{
