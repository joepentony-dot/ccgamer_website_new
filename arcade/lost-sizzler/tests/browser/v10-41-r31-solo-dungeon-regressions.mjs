...
try{
  ...
  const cpuAndHud=await page.evaluate(()=>{
    const cpu=host.enemies.find(enemy=>enemy?._v141R31NamedCpuCook),node=document.getElementById("hud-score"),style=getComputedStyle(node),rect=node.getBoundingClientRect();
    ...
    return{controller:window.CCGLostSizzlerModeRuntime.detect(),cpuName:cpu?.follower?.name||"",cpuInitials:cpu?.follower?.initials||"",cpuKind:cpu?.follower?.kind||"",named:Boolean(cpu?._v141R31NamedCpuCook),portraitAlias,visibility:style.visibility,display:style.display,opacity:style.opacity,width:rect.width,height:rect.height};
  });
