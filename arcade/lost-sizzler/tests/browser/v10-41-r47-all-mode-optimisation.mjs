...
try{
  ...
  console.log("[r47 active modes] load canonical runtime and governor");
  await page.goto(`${origin}/arcade/lost-sizzler/?r47-active-modes=1`,{waitUntil:"domcontentloaded"});
  ...
  const coverage=await page.evaluate(()=>{
    ...
    const modes=["solo","split","daily","tutorial","dungeon"];
  ...
  console.log("[r47 active modes] pressure thresholds and client fault fingerprints are deterministic");
  ...
  console.log("[r47 active modes] Solo runtime keeps gameplay arrays intact while shedding only disposable visuals");
  ...
  console.log("[r47 active modes] pause/resume and resize abuse remain stable under the governor");
  ...
  console.log("[r47 active modes] bounded soak cycles diagnostics without installing extra gameplay ownership");
  ...
  console.log("Lost Sizzler V10.41 r47 active-mode governor, gameplay ownership, Split/Solo and bounded-soak checks passed in Chromium.");
  await context.close();
}finally{
  await browser.close();
