...
try{
  ...
  assert.ok(buildMatch&&Number(buildMatch[1])>=23,"Ordered bootstrap must advertise r23 or a later V10.42 build.");
  assert.ok(cacheMatch&&Number(cacheMatch[1])>=23,"Ordered bootstrap must use an r23-or-later module cache token.");
  assert.equal(cacheMatch[1],buildMatch[1],"Ordered bootstrap build and cache revisions must stay aligned.");
  ...
  const persisted=await page.evaluate(()=>({floor:run.floor,cap:window.CCGProgression.floorLevelCap(run),rpg:{...p1.rpgStats},milestones:{...p1.v142R23BuildMilestones},maxHealth:p1.maxHealth,maxMana:p1.maxMana,mana:p1.mana,armor:p1.armor,moveMultiplier:p1.moveMultiplier,dashDamage:p1.dashDamage,sight:p1.v142SightBonus,ward:p1.v142WardCooldownMs}));
  ...
  assert.equal(persisted.maxMana,294,"Endurance specialization capacity must survive the legacy V10.25 floor-transition ammo clamp without stacking.");
  assert.equal(persisted.mana,294,"Endurance specialization current ammo must survive the real floor transition without an implicit refill or clamp.");
