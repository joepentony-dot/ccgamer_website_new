...
try{
  ...
  await page.waitForFunction(()=>document.body.dataset.releaseReady==="true"&&Boolean(window.CCGLostSizzlerV142R23RpgBuildExpansion)&&Boolean(window.CCGLostSizzlerV142ProceduralOverhaul)&&Boolean(window.CCGLostSizzlerV142FiveDepthCampaign),null,{timeout:90000});
  ...
  assert.ok(bootstrap.loaded.includes("v10-42-r23-rpg-build-expansion.js"),"Ordered bootstrap must load the r23 RPG expansion.");
  ...
  await page.evaluate(()=>{try{window.CCGProgression?.clearCheckpoint?.()}catch(_){}});
  console.log("V10.42 r23 RPG specialisations → Floor 2 persistence qualification passed.");
  await context.close();
}finally{
