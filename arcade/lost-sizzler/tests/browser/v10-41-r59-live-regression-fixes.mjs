...
try{
  ...
  assert.deepEqual(errors,[],`R59 Solo live-regression browser test produced page errors: ${errors.join("\n")}`);
  console.log(`Lost Sizzler V10.41 R59 Solo pause cadence (${baselineFrames}->${afterFrames}, ratio ${ratio.toFixed(2)}) and paused-gap isolation passed in Chromium.`);
  await context.close();
}finally{
