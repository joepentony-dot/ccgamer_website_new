...
try{
  ...
  await page.waitForFunction(()=>Boolean(window.CCGLostSizzlerV141R29SpyEngine&&window.CCGLostSizzlerSaboteurs&&window.CCGLostSizzlerSpecialModes));

  const entered=await page.evaluate(()=>{
    const special=window.CCGLostSizzlerSpecialModes,SAB=window.CCGLostSizzlerSaboteurs,engine=window.CCGLostSizzlerV141R29SpyEngine;
    ...
    return{isolated:engine.state.isolated,spyBelow:Boolean(before?.__ccgV141SpyDamageBoundary)};
  });
  ...
  assert.equal(retained.topProbe,true,`R29 monitor must not discard a later owner that composes around the Spy boundary: ${JSON.stringify(retained.chain)}`);
  ...
  console.log("Lost Sizzler V10.41 Spy damage ownership preserved later wrappers across monitor and mode exit.");
  await context.close();
}finally{
