...
try{
  ...
  await page.evaluate(()=>{
    ...
    const sample=()=>{
      ...
      window.__ccgLoadingProgressSamples.push({
        ...
        orderedLoaded:Number(watchdog?.state?.orderedLoaded||0),
        orderedTotal:Number(watchdog?.state?.orderedTotal||0),
        currentModule:String(watchdog?.state?.currentModule||""),
  ...
  const finished=await page.evaluate(()=>{
    ...
    return{
      ...
      watchdog:window.CCGLostSizzlerLoadWatchdog?{
        ...
        orderedLoaded:Number(window.CCGLostSizzlerLoadWatchdog.state?.orderedLoaded||0),
        orderedTotal:Number(window.CCGLostSizzlerLoadWatchdog.state?.orderedTotal||0),
        currentModule:String(window.CCGLostSizzlerLoadWatchdog.state?.currentModule||""),
  ...
  assert.ok(finished.watchdog?.orderedTotal>0,`the V10.42 ordered bootstrap must publish its actual workload: ${JSON.stringify(finished.watchdog)}`);
  assert.equal(finished.watchdog?.orderedLoaded,finished.watchdog?.orderedTotal,`all ordered V10.42 modules must complete before loader dismissal: ${JSON.stringify(finished.watchdog)}`);
  assert.equal(finished.watchdog?.stage,100,"the authoritative loader stage must finish at 100");
  const preReady=finished.samples.filter(sample=>!sample.releaseReady);
  ...
  const ordered=preReady.filter(sample=>sample.orderedTotal>0);
  assert.ok(ordered.length>=2,`ordered V10.42 loading must expose multiple live samples: ${JSON.stringify(preReady)}`);
  assert.ok(ordered.every(sample=>sample.orderedTotal===finished.watchdog.orderedTotal),"ordered module denominator must remain stable once V10.42 bootstrap begins");
  for(let i=1;i<ordered.length;i++){
    const previous=ordered[i-1],current=ordered[i];
    assert.ok(current.orderedLoaded>=previous.orderedLoaded,`ordered completion count must never move backwards: ${JSON.stringify({previous,current})}`);
    if(current.value>previous.value)assert.ok(current.orderedLoaded>previous.orderedLoaded,`ordered loading percentage must only rise when another V10.42 module completes: ${JSON.stringify({previous,current})}`);
  }
