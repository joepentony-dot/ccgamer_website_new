...
try{
  ...
  const finished=await page.evaluate(()=>({
    ...
    retiredHorde:Boolean(document.getElementById("horde-mode-btn")),
    retiredSpy:Boolean(document.getElementById("saboteurs-mode-btn"))
  }));
  ...
  assert.equal(finished.gameReady,"true","game readiness must be established after the release gate completes");
  assert.equal(finished.build,finished.canonicalBuild,"page build metadata must match the authoritative V10.42 bootstrap");
  assert.equal(finished.cache,finished.canonicalCache,"page cache metadata must match the authoritative V10.42 bootstrap");
  assert.equal(finished.retiredHorde,false,"retired Horde Multiplayer control must stay absent after loading");
  assert.equal(finished.retiredSpy,false,"retired Spy Vs Spy Multiplayer control must stay absent after loading");

  assert.deepEqual(errors,[],`active loading launch must have no uncaught browser errors: ${errors.join("\n")}`);
  console.log("C64 Dungeon Carnage active loading progress and V10.42 release readiness passed in Chromium.");
  await context.close();
}finally{
  await browser.close();
