...
try{
  ...
  await page.waitForFunction(()=>document.body.dataset.releaseReady==="true"&&Boolean(window.CCGLostSizzlerModeRuntime)&&Boolean(window.CCGLostSizzlerSpecialModes)&&Boolean(window.CCGLostSizzlerV141R29SpyEngine)&&Boolean(window.CCGLostSizzlerV141R32SpyLoader)&&typeof quitToMenu==="function",null,{timeout:90000});

  const snapshot=()=>page.evaluate(()=>{
    const runtime=window.CCGLostSizzlerModeRuntime,r29=window.CCGLostSizzlerV141R29SpyEngine,r32=window.CCGLostSizzlerV141R32SpyLoader,r56=window.CCGLostSizzlerV141R56PlaytestCompletion,r59=window.CCGLostSizzlerV141R59LiveRegressionFixes,r60=window.CCGLostSizzlerV141R60HordeCombatIntegrity;
    ...
    return{
      ...
      r29Timer:Number(r29?.state?.timer||0),worldBuilds:Number(r29?.state?.worldBuilds||0),logicalCompactions:Number(r29?.state?.logicalCompactions||0),controllerFrames:Number(r29?.state?.controllerFrames||0),moveReassertions:Number(r29?.state?.moveReassertions||0),updateReassertions:Number(r29?.state?.updateReassertions||0),
      ...
      moveDepth:move.depth,moveOwners:move.moveOwners,hurtDepth:hurt.depth,damageBoundaries:hurt.damageBoundaries,
      inventoryOpen:Boolean(overhaul?.state?.inventoryOpen),searchPending:Boolean(overhaul?.state?.search),
      r56TrapHits:Number(r56?.state?.trapHits||0),r56EnvironmentHits:Number(r56?.state?.environmentHits||0),r56CombatRearms:Number(r56?.state?.combatRearms||0),
      r59SoloFrames:Number(r59?.state?.soloFrames||0),r59SoloSubsteps:Number(r59?.state?.soloSubsteps||0),r60HordeFrames:Number(r60?.state?.frames||0)
    }
  });
  ...
  for(let cycle=1;cycle<=3;cycle++){
    ...
    assert.ok(entry.r29Timer>0,`Spy entry ${cycle} must reuse the established bounded r29 monitor handle`);
  ...
  for(const [index,entry] of entries.entries()){
    ...
    assert.equal(entry.r29Timer,entries[0].r29Timer,`Spy re-entry ${index+1} must reuse the r29 monitor handle`);
  ...
  console.log("Lost Sizzler Stage 10 real Spy launch, lazy-loader, isolated-owner and repeated re-entry qualification passed in Chromium.");
  await context.close();
