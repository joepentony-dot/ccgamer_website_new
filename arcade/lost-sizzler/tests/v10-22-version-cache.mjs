...
assert.equal(manifest.build,"2026.08.24.2","current published build must be explicit in the regression check");
assert.equal(manifest.cacheToken,"20260824r2","current release cache token must be explicit in the live manifest");
...
assert.match(index,/js\/v10-23-tutorial-guidance\.js\?v=20260824c/,"r2 must use the fullscreen-safe tutorial launcher revision");
assert.ok(index.indexOf("js/v10-23-tutorial-guidance.js?v=20260824c")<index.indexOf("js/asset-overrides.js?v=20260824r2"),"tutorial launch guidance must load before the enhancement queue");
assert.match(index,/id="hud-mana">0\/120</,"r2 static HUD must reflect the sword-first 120-round ammunition model");
assert.match(index,/id="hud-weapon">SWORD</,"r2 static HUD must show the starting melee weapon before runtime sync");
assert.match(index,/SPACE ATTACK/,"r2 keyboard help must describe the shared firearm/melee attack action");
...
console.log("Lost Sizzler r2 build, keyboard/tutorial/voice cache refresh, canonical home recognition and scroll-safety regression checks passed.");
