...
assert.match(reporter,/recordTrapDamage\(detail=\{\}\)\{return recordEnvironmentDamageSignal\("trap",\{detail\}\)\}/,"reporter must expose a direct exact trap-damage intake for authoritative runtime owners");
...
assert.match(gamePlay,/const detail=\{playerId:String\(p\.id\|\|p\.name\|\|"P1"\),trapId:String\(t\.id\|\|\`\$\{t\.x\},\$\{t\.y\}\`\),kind:String\(t\.kind\|\|"floor"\),x:Number\(t\.x\),y:Number\(t\.y\),at:damageAt\}[\s\S]*recordTrapDamage\?==="function"[\s\S]*recordTrapDamage\(detail\)[\s\S]*new CustomEvent\("ccg:trap-damage",\{detail\}\)/,"ordinary traps must publish exact accepted-damage contact evidence directly with an event fallback");
...
assert.match(r19,/function recordTrapHit\(player,trap,contactKey,cycle\)[\s\S]*const detail=\{playerId:playerId\(player\),trapId:trapId\(trap\),kind,x:Number\(trap\.x\),y:Number\(trap\.y\)[\s\S]*recordTrapDamage\?==="function"[\s\S]*recordTrapDamage\(detail\)[\s\S]*new CustomEvent\("ccg:trap-damage",\{detail\}\)/,"the authoritative R19 trap owner must publish exact accepted player/trap evidence directly with an event fallback");
assert.match(gamePlay,/if\(healthLost&&damageAt>beforeDamageAt\)[\s\S]*recordTrapDamage\(detail\)[\s\S]*new CustomEvent\("ccg:trap-damage",\{detail\}\)/,"canonical triggerTrap must publish exact accepted-damage contact evidence after real health loss");
