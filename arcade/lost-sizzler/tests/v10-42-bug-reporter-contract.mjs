...
assert.match(reporter,/boundaryDamageSignalSerial=environmentDamageSerial[\s\S]*boundaryContactSignals=contactDamageSignalsSince\(before,boundaryDamageSignalSerial\)[\s\S]*boundaryTrapSignal=boundaryContactSignals\.find\(signal=>signal\.type==="trap"[\s\S]*boundaryTrapIds\.has\(signal\.trapId\)/,"ordinary trap confirmation must seal the exact accepted trap signal at the movement boundary before a dash can leave the tile");
...
assert.match(gamePlay,/if\(!routed&&healthLost&&damageAt>beforeDamageAt\)[\s\S]*new CustomEvent\("ccg:trap-damage"/,"canonical triggerTrap must emit fallback contact evidence only when R19 did not route the hit");
