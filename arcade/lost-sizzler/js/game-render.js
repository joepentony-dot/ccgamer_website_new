const lostSizzlerPixelAssets=(()=>{const make=src=>{if(typeof Image!=="function")return null;const image=new Image();image.decoding="async";try{image.fetchPriority="high"}catch(_){}image.src=src;try{image.decode?.().catch(()=>{})}catch(_){}return image};return{explorer:make("assets/pixel/explorer-sheet-v10-34.png?v=20260824r8"),chests:make("assets/pixel/chest-sheet-v10-34.png?v=20260824r8")}})();
...
function drawTile(x,y){
  ...
  const h=tileHash(x,y,variant+roomId),theme=room?.theme||"";ctx.fillStyle=(x+y+variant)%2?th.floor:th.alt;ctx.fillRect(s.x,s.y,C.tile,C.tile);if(roomId<0)try{window.CCGLostSizzlerV142R54VisualGameplay?.drawCorridorTile?.(ctx,{s,x,y,hash:h,theme:th,C,floor:run?.floor||1})}catch(_){}
...
function drawPickupGlyph(i,col){
  ...
  const k=i.kind;
  const custom=pickupOverrideImages.get(k);if(custom?.complete&&custom.naturalWidth){ctx.drawImage(custom,-14,-14,28,28);ctx.restore();return}if(window.CCGLostSizzlerV142R54VisualGameplay?.drawPickupGlyph?.(ctx,i,col,P)){ctx.restore();return}
...
function drawItem(i){
  ...
  if(md(i,focus)<=3){const logical=window.CCGLostSizzlerV142R54VisualGameplay?.pickupLabel?.(i);label(logical||i.title||({health:"POTION",ammo:"AMMO",exitSigil:"EXIT SIGIL",key:"MAIN KEY",game:"C64 GAME",credits:"SCORE COIN",xpOrb:"10 XP",torch:"TORCH",teleport:"TELEPORT SPELL",armour:"ARMOUR +1",potion:"POTION",bronze:"BRONZE KEY",weapon:"WEAPON CACHE",rapid:"RAPID FIRE"}[i.kind]||i.kind.toUpperCase()),{x:s.x,y:s.y-1},col)}
}
function drawChests(){
  const now=performance.now();
...
function drawPixelEnemySprite(e,cx,cy){
  const f=e.follower,k=f?.kind||e.kind||"guardian",isDeath=Boolean(e.deathStalker&&e.voidStalker),elite=Boolean(f||e.champion||e.guardian||e.exitWarden),seed=enemySpriteSeed(e),now=performance.now(),walking=e.aiState==="chase"||e.aiState==="search",attacking=(e.meleeSwingMs||0)>0||(e.chargeTelegraphMs||0)>0,anim=window.CCGLostSizzlerV142R54VisualGameplay?.enemyFrame?.({now,walking,hit:(e.hitStunMs||0)>0,attacking,seed})||null,phase=anim?anim.phase:now/210+(seed%37),stride=anim?anim.stride:(walking?Math.sin(phase*1.35):Math.sin(phase*.38)*.25),bob=anim?anim.bob:(walking?Math.round(Math.abs(stride)*-2):Math.round(Math.sin(phase*.45))),facing=(e.facing?.x||(seed%2?1:-1))<0?-1:1,pal=enemyPalette(k,e);
...
function drawPlayer(p,kind="p1"){
  ...
  if(sheet?.complete&&sheet.naturalWidth>=192){const row=d.y>0?0:d.x>0?1:d.y<0?2:3,anim=window.CCGLostSizzlerV142R54VisualGameplay?.playerFrame?.({now,moving,swingActive,swingAge,swingMs,hurt,seed:String(p.id||kind).length*37})||{column:hurt?5:swingActive?(swingAge/swingMs<.42?3:4):moving?(Math.floor(now/145)%2?1:2):0,dx:0,dy:0,scaleX:1,scaleY:1};ctx.save();ctx.translate(s.x+C.tile/2+Number(anim.dx||0),s.y+C.tile/2+bob+Number(anim.dy||0));ctx.scale(Number(anim.scaleX||1),Number(anim.scaleY||1));ctx.drawImage(sheet,Number(anim.column||0)*32,row*32,32,32,-C.tile/2,-C.tile/2,C.tile,C.tile);ctx.restore();if(!hurt&&(!swingActive||hasGun))drawPlayerWeapon(p,cx+Number(anim.dx||0),cy+Number(anim.dy||0),d);}else{
...
function drawSpecialObjects(){
  const shops=host.shops?.length?host.shops:[host.trader].filter(Boolean);for(const t of shops)if(t?.active&&visibleTo(focus,t.x,t.y)){t.discovered=true;const s=ws(t.x,t.y),entrance=t.shopType==="entrance",col=entrance?P.cyan:P.gold,merchantDrawn=window.CCGLostSizzlerV142R54VisualGameplay?.drawMerchant?.(ctx,t,s,col,entrance,P,C,performance.now());if(!merchantDrawn){ctx.save();ctx.translate(s.x+C.tile/2,s.y+C.tile/2);ctx.strokeStyle=col;ctx.fillStyle="#10222b";ctx.shadowColor=col;ctx.shadowBlur=12;ctx.lineWidth=2;ctx.fillRect(-14,-9,28,19);ctx.strokeRect(-14,-9,28,19);ctx.fillStyle=P.gold;ctx.fillRect(-12,-6,24,4);ctx.fillStyle=col;ctx.fillRect(-10,1,6,5);ctx.fillRect(4,1,6,5);ctx.font='bold 6px "Courier New"';ctx.textAlign="center";ctx.fillText("SHOP",0,9);ctx.restore()}if(md(t,focus)<=3){const merchant=t.stage7NpcName?` — ${String(t.stage7NpcName).toUpperCase()}`:"";label(entrance?`FLOOR SUPPLY DESK${merchant} — STEP ON TO SHOP`:`SECRET DUNGEON SHOP${merchant} — TRADE OR BUY`,s,col)}}
