...
(function installSaboteursAudio(root, factory) {
  ...
  function createController(options = {}) {
    ...
    async function start() {
      ...
      root?.CCGLostSizzlerMusicBus?.claim?.("sizzler-saboteurs", () => stopImmediate(false));
    ...
    function stop(reset = true) {
      ...
      setTimeout(() => { if (!audio) return; try { audio.pause(); if (reset) audio.currentTime = 0; } catch (_) {} playing = false; root?.CCGLostSizzlerMusicBus?.release?.("sizzler-saboteurs"); }, config.fadeOutMs + 40);
      return true;
    }

    function stopImmediate(releaseOwnership = true) {
      clearDuck(); clearFade(); if (audio) { try { audio.volume = 0; audio.pause(); } catch (_) {} } playing = false;
      if (releaseOwnership) root?.CCGLostSizzlerMusicBus?.release?.("sizzler-saboteurs"); return true;
    }
    ...
    function dispose() {
      stopImmediate(); if (audio) { try { audio.currentTime = 0; } catch (_) {} }
      audio = null; playing = false;
    }

    return Object.freeze({
      start, stop, stopImmediate, duck, setEnabled, setVolume, dispose,
