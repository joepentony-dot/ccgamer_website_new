...
(()=>{
  ...
  function setBuildLabel(){
    document.querySelectorAll(".build-badge").forEach(node=>node.textContent="BUILD V10.34");
    const subtitle=document.querySelector(".brand p");if(subtitle)subtitle.textContent="THE LOST SIZZLER — V10.34";
  }

  function keepSubtitleCurrent(){
    const subtitle=document.querySelector(".brand p");if(!subtitle)return;
    const update=()=>{if(subtitle.textContent!=="THE LOST SIZZLER — V10.34")subtitle.textContent="THE LOST SIZZLER — V10.34"};
