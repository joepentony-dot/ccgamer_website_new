...
function triggerMemoryPuzzle(p,deliberate=false){const z=host.memoryPuzzle;if(!z||z.solved||!deliberate)return;const activator=z.activator;if(activator&&activator.x===p.x&&activator.y===p.y){startMemoryPuzzle(p);return}if(z.phase!=="input")return;const t=(z.tiles||[]).find(t=>t.x===p.x&&t.y===p.y);if(t)activateMemoryTile(p,t.index)}
function updateMemoryPuzzle(dt){const z=host.memoryPuzzle;if(!z||z.solved)return;const activator=z.activator,onConsole=activator?localPlayers().find(p=>p&&p.health>0&&p.x===activator.x&&p.y===activator.y):null,occupant=String(onConsole?.id||"");if(occupant&&occupant!==String(z.consoleOccupant||"")){z.consoleOccupant=occupant;startMemoryPuzzle(onConsole)}else if(!occupant&&z.consoleOccupant)z.consoleOccupant="";if(z.phase!=="show")return;z.flashElapsed=(z.flashElapsed||0)+dt;const step=760,idx=Math.floor(z.flashElapsed/step),within=z.flashElapsed%step;z.flashTile=idx<z.sequence.length&&within<500?z.sequence[idx]:-1;if(z.flashElapsed>=z.sequence.length*step+420){z.phase="input";z.flashTile=-1;z.inputIndex=0;S.sfx("pickup");showToast("YOUR TURN","Repeat the five numbered pads. The centre of the room is now completely safe to cross. If you forget the order, step on the purple replay console — no penalty.","cyan",7600);host.revision++;broadcastWorld()}}
