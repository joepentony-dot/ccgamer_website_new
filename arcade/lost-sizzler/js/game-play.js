...
function startMemoryPuzzle(p){const z=host.memoryPuzzle;if(!z||z.solved||W.roomAt(world,p.x,p.y)!==z.roomId)return false;z.phase="show";z.flashElapsed=0;z.flashTile=-1;z.inputIndex=0;host.revision++;S.sfx("shrine");showToast("MEMORY PAD SEQUENCE","Watch the five numbered pads. Step on them in the same order when the replay finishes. Return to the purple console whenever you want to see the sequence again.","cyan",10000);broadcastWorld();return true}
function memoryConsoleContact(p){const z=host.memoryPuzzle;if(!z||z.solved||!p||!z.activator)return false;z.consoleOccupants=Array.isArray(z.consoleOccupants)?z.consoleOccupants:[];const id=String(p.id||"p"),on=z.activator.x===p.x&&z.activator.y===p.y,had=z.consoleOccupants.includes(id);if(!on){if(had)z.consoleOccupants=z.consoleOccupants.filter(value=>value!==id);return false}if(had)return false;z.consoleOccupants.push(id);return startMemoryPuzzle(p)}
...
function triggerMemoryPuzzle(p,deliberate=false){const z=host.memoryPuzzle;if(!z||z.solved||!deliberate)return;if(memoryConsoleContact(p))return;if(z.phase!=="input")return;const t=(z.tiles||[]).find(t=>t.x===p.x&&t.y===p.y);if(t)activateMemoryTile(p,t.index)}
function updateMemoryPuzzle(dt){const z=host.memoryPuzzle;if(!z||z.solved)return;for(const player of localPlayers())memoryConsoleContact(player);if(z.phase!=="show")return;z.flashElapsed=(z.flashElapsed||0)+dt;const step=760,idx=Math.floor(z.flashElapsed/step),within=z.flashElapsed%step;z.flashTile=idx<z.sequence.length&&within<500?z.sequence[idx]:-1;if(z.flashElapsed>=z.sequence.length*step+420){z.phase="input";z.flashTile=-1;z.inputIndex=0;S.sfx("pickup");showToast("YOUR TURN","Repeat the five numbered pads. The centre of the room is now completely safe to cross. If you forget the order, step on the purple replay console — no penalty.","cyan",7600);host.revision++;broadcastWorld()}}
