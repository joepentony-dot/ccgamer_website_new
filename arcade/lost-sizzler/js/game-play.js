...
function startMemoryPuzzle(p){const z=host.memoryPuzzle;if(!z||z.solved||W.roomAt(world,p.x,p.y)!==z.roomId)return false;z.phase="show";z.flashElapsed=0;z.flashTile=-1;z.inputIndex=0;z.consoleOccupant=String(p.id||"");z.replayToken=(z.replayToken||0)+1;host.revision++;S.sfx("shrine");showToast("MEMORY PAD SEQUENCE","Watch the five numbered pads. The replay is shown once per console entry; when it finishes, repeat the pads in the same order. Step off and back onto the purple console to replay it.","cyan",10000);broadcastWorld();return true}
...
function activateMemoryTile(p,tileIndex){const z=host.memoryPuzzle;if(!z||z.solved||z.phase!=="input")return false;const expected=z.sequence[z.inputIndex];if(tileIndex===expected){z.inputIndex++;S.sfx("pickup");floatText(p.x,p.y,`${z.inputIndex}/${z.sequence.length}`,P.cyan);if(z.inputIndex>=z.sequence.length)solveMemoryPuzzle(p);else host.revision++;return true}z.failures=(z.failures||0)+1;z.inputIndex=0;z.phase="idle";z.flashElapsed=0;z.flashTile=-1;const spawned=spawnPuzzleAmbush(z.roomId,p,1,"memory-fail");S.sfx("alert");showToast("WRONG MEMORY PAD",`That was not the next pad. ${spawned} wandering monster${spawned===1?"":"s"} enters the chamber. The sequence is now paused — step on the purple replay console when you are ready to try again.`,"red",9500);host.revision++;broadcastWorld();return false}
function triggerMemoryPuzzle(p,deliberate=false){const z=host.memoryPuzzle;if(!z||z.solved||!deliberate)return;const activator=z.activator;if(activator&&activator.x===p.x&&activator.y===p.y){if(String(z.consoleOccupant||"")!==String(p.id||""))startMemoryPuzzle(p);return}if(z.phase!=="input")return;const t=(z.tiles||[]).find(t=>t.x===p.x&&t.y===p.y);if(t)activateMemoryTile(p,t.index)}
function updateMemoryPuzzle(dt){const z=host.memoryPuzzle;if(!z||z.solved)return;const activator=z.activator,onConsole=activator?localPlayers().find(p=>p&&p.health>0&&p.x===activator.x&&p.y===activator.y):null,occupant=String(onConsole?.id||"");if(occupant&&occupant!==String(z.consoleOccupant||""))startMemoryPuzzle(onConsole);else if(!occupant&&z.consoleOccupant)z.consoleOccupant="";if(z.phase!=="show")return;z.flashElapsed=(z.flashElapsed||0)+Math.max(0,Number(dt||0));const step=760,idx=Math.floor(z.flashElapsed/step),within=z.flashElapsed%step;z.flashTile=idx<z.sequence.length&&within<500?z.sequence[idx]:-1;if(z.flashElapsed>=z.sequence.length*step+420){z.phase="input";z.flashTile=-1;z.inputIndex=0;S.sfx("pickup");showToast("YOUR TURN","Repeat the five numbered pads. Only deliberate steps onto numbered pads count. If you forget the order, step off and back onto the purple replay console — no penalty.","cyan",7600);host.revision++;broadcastWorld()}}
...
function movementTriggers(p,deliberate=false){triggerSwitch(p);triggerTrader(p);triggerDeathCache(p);triggerBloodClue(p);triggerMemoryPuzzle(p,deliberate);triggerSequenceTorch(p);triggerWeightBridge(p);triggerShrine(p);triggerTrap(p);triggerRescue(p);triggerArena(p);triggerTimed(p);triggerBoulder(p);triggerHauntedCorridor(p);triggerSigilRoom(p);markRoomVisit(p);rememberTrail(p);try{window.CCGLostSizzlerStage8NpcDialogue?.onMovementBoundary?.(p)}catch(_){}}
function movePlayer(p,dx,dy,dash=false){
  ...
  for(let n=0;n<(dash?2:1);n++){
    ...
    const warp=(!dy)?W.tunnelDestination(world,p.x,p.y,dx,dy):null;if(warp){p.x=warp.x;p.y=warp.y;p.rx=warp.x;p.ry=warp.y;S.sfx("warp");burst(p.x,p.y,P.cyan,18,1.2);movementTriggers(p,true);continue}
    ...
    updateRoomMessage(p,false);reveal(p);movementTriggers(p,true)
  }
