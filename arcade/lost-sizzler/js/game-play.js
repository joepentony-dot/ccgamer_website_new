...
function triggerTrap(p){const now=performance.now();for(const t of host.traps||[])if(t.active&&t.x===p.x&&t.y===p.y&&SYS.trapActive(t,now)){const beforeHealth=Number(p.health||0),beforeArmor=Number(p.armor||0);S.sfx("trap");showToast(`${t.kind.toUpperCase()} TRAP`,`The floor was trying to tell you something. -1 health.`,"red");hurtPlayer(p,1,false,`${t.kind} trap`);window.CCGLostSizzlerV142R19MobileTrapLayoutStability?.guaranteeTrapContactDamage?.(p,t,beforeHealth,beforeArmor)}}
