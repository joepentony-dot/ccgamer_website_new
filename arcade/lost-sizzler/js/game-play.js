...
function hurtPlayer(p,n,friendly=false,source="enemy"){
  ...
  if(p.health<=0){
    ...
    const deathRoomReopened=releaseSealedDeathRoom(W.roomAt(world,deathX,deathY));
    ...
    showToast(penalty.zeroWarning?`${p.name.toUpperCase()} RESPAWNS — FINAL XP WARNING`:`${p.name.toUpperCase()} RESPAWNS — SCORE HALVED`,`OBJECTIVE: ${objective}.${xpText}${cacheText}${zeroText}`,"red",penalty.zeroWarning?13000:10000);host.revision+=deathRoomReopened?2:1;broadcastWorld();if(run.consecutiveDeaths>=5)setTimeout(()=>{if(mode==="playing")offerFloorSave(true)},650)
  }sync()
}
