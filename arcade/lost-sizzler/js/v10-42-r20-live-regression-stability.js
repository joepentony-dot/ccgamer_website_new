...
(()=>{
  ...
  function livePlayerBulletCount(player){
    try{return (bullets||[]).filter(projectile=>projectile&&projectile.ttl>0&&projectile.owner===player?.id).length}catch(_){return 0}
  }

  function shotCompleted(player,beforeMana,beforeBullets){
    const mana=Math.max(0,Number(player?.mana)||0);
    const activeBullets=livePlayerBulletCount(player);
    return mana<beforeMana||activeBullets>beforeBullets
  }
  ...
  function recoverPersistentFireBlock(player,direction,beforeMana,beforeBullets){
    ...
    if(livePlayerBulletCount(player)>=max)return false;
  ...
  function attackNow(code){
    ...
    const beforeBullets=livePlayerBulletCount(player);
