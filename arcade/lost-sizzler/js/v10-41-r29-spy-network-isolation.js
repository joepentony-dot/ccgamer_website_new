...
(()=>{
  ...
  const MODE_ID="sizzler-saboteurs",PACKET="v141_spy_position",SEND_MS=85,MONITOR_MS=40;
  const state={installed:false,timer:0,lastSendAt:0,basePacket:null,sent:0,received:0,reassertions:0,dropped:0,lastRemoteAt:0};
  ...
  function monitor(){
    if(!net?.cb)return;
    if(spyActive()){
      if(!state.installed)install();else reassert();sendPosition(false);return;
    }
    if(state.installed)restore();
  }

  // Movement uses an immediate position publish as well as the 85 ms heartbeat,
  // reducing visible latency without bringing the Dungeon update back into Spy.
  ...
  state.timer=setInterval(()=>{monitor();wrapAttemptMove()},MONITOR_MS);monitor();wrapAttemptMove();
  addEventListener("pagehide",()=>{if(state.timer)clearInterval(state.timer);restore()},{once:true});

  window.CCGLostSizzlerV141R29SpyNetwork={PACKET,sendPosition,applyPosition,install,restore,get state(){return state}};
})();