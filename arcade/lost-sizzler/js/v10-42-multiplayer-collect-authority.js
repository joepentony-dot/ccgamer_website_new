...
(()=>{
  ...
  function install(){
    ...
    const wrapped=function onCollectRequestV142Authority(payload){
      ...
      if(!remoteCollector||!domain||!wasActive||item?.active!==false||!runState||beforeDomains.has(domain.id)||runState.v142ClaimedDomains?.includes?.(domain.id))return result;
