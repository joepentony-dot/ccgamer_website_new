...
(()=>{
  ...
  const state={forceTutorial:false,choiceAccepted:false,tutorialRequested:false,active:false,step:0,start:null,moved:false,movementDistance:0,lastMovement:new Map(),fired:false,dashed:false,inventoryOpened:false,inventoryClosed:false,pendingBegin:null,lockedDoors:[],panel:null,choice:null,welcomeForRun:false,autoAdvanceTimer:0,completionBanner:null,installed:{begin:false,startWorld:false,actions:false,dossier:false}};
  ...
  function completeInteractive(kind){
    ...
    state.autoAdvanceTimer=setTimeout(()=>{state.autoAdvanceTimer=0;if(state.active&&STEPS[state.step]?.[0]===kind&&stepReady(STEPS[state.step]))advance()},220);return true;
  }
  ...
  function handleTutorialKeydown(event){
    ...
    if(step==="fire"&&(event.code==="Space"||event.code==="Enter")){completeInteractive("fire");return}
    if(step==="dash"&&["ShiftLeft","ShiftRight","ControlLeft","ControlRight"].includes(event.code)){completeInteractive("dash");return}
  ...
  function handleTutorialPointer(event){
    ...
    if(step==="fire"&&action==="fire"){completeInteractive("fire");return}
    if(step==="dash"&&action==="dash"){completeInteractive("dash");return}
