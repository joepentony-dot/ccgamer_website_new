window.CCGSystems=(()=>{
  ...
  function installDedicatedHazardRooms(world,host,run,rooms,used){
    ...
    const hazardEligible=(room,minW=8,minH=7)=>Boolean(room&&!room.sanctuary&&!room.sigilRoom&&!room.spiderNest&&!busy.has(room.id)&&room.id!==world.startRoomId&&room.id!==world.exitRoomId&&room.w>=minW&&room.h>=minH),primaryHazardRooms=rooms.filter(room=>hazardEligible(room)),fallbackHazardRooms=(world.rooms||[]).filter(room=>hazardEligible(room)&&!primaryHazardRooms.some(candidate=>candidate.id===room.id)),relaxedHazardRooms=primaryHazardRooms.length||fallbackHazardRooms.length?[]:(world.rooms||[]).filter(room=>hazardEligible(room,6,5)),choices=[...primaryHazardRooms,...fallbackHazardRooms,...relaxedHazardRooms].map(room=>({room,key:world.random()})).sort((a,b)=>a.key-b.key),types=["blade","embers","arrows"];host.hazardRooms=[];
