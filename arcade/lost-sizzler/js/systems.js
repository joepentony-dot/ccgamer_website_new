window.CCGSystems=(()=>{
  ...
  function decorate(world,host,run){
    ...
    const hazardReserveSort=list=>[...list].sort((a,b)=>(b.w*b.h)-(a.w*a.h)||Number(b.depth||0)-Number(a.depth||0)||Number(a.id)-Number(b.id));
    const hazardReserveLarge=hazardReserveSort(rooms.filter(room=>room&&room.id!==world.startRoomId&&room.id!==world.exitRoomId&&room.w>=6&&room.h>=5));
    const hazardReserveLargeIds=new Set(hazardReserveLarge.map(room=>room.id));
