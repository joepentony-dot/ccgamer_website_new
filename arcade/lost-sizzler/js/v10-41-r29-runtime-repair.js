/* The Lost Sizzler V10.41 r29 — active runtime compatibility. */
(()=>{
  "use strict";
  ...
  function installContactCombatGuard(){
    ...
    if(current===state.lastContactSource)return state.contactInstalled;
  ...
  function normaliseItemNames(){
    ...
    state.itemNameHost=hostState;state.itemNameRevision=revision;state.itemNamesNormalised+=changed;return changed;
  }
  ...
  function drawEnhancedPickup(item,col){
    ...
    ctx.restore();state.pickupEnhancedDraws++;return true;
  }
  function installPickupGraphics(){
    ...
    if(current===state.lastPickupSource)return state.pickupInstalled;
  ...
  function sealRoomDoorBypasses(worldState,hostState){
    ...
    if(sealed){hostState.revision=(Number(hostState.revision)||0)+1;state.doorBypassesSealed+=sealed;try{if(typeof broadcastWorld==="function"&&(typeof playMode==="undefined"||playMode!=="online"||net?.isHost))broadcastWorld()}catch(_){}}
    return sealed;
  }
  function repairDungeonStructure(){
    const worldState=typeof world!=="undefined"?world:null,hostState=typeof host!=="undefined"?host:null;return sealRoomDoorBypasses(worldState,hostState)
  }
  ...
  function handleEnterProgress(event){
    ...
    event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();try{button.focus({preventScroll:true})}catch(_){}button.click();state.enterContinues++;
  }
