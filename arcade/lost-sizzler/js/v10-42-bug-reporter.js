...
(()=>{
  ...
  function fireProbe(code,before){
    ...
    const validFirearmAttempt=Boolean(
      before.player1.firearmUnlocked&&before.player1.weapon&&Number(before.player1.mana)>0&&
      Number(before.player1.hitStunMs)<=0&&Number(before.game.activeProjectiles)<Number(before.game.maxProjectiles||Infinity)
    );
    setTimeout(()=>{
      ...
      if(!fired&&after.game.mode==="playing"&&after.game.runActive&&after.browser.visibility==="visible"){
        ...
        push("ANOMALY_POSSIBLE_ATTACK_FAILURE",details);
        if(validFirearmAttempt)push("ANOMALY_POSSIBLE_FIRE_FAILURE",details);
        updateBadge();
      }
    },900);
  }
