...
(()=>{
  ...
  function install(){
    ...
    canvas.style.filter="saturate(1.13) contrast(1.055) brightness(.985)";
    layer.style.cssText+=";position:absolute;inset:0;z-index:1;pointer-events:none;--r51-light-x:50%;--r51-light-y:50%;--r51-ambient-rgb:142,92,255;background:radial-gradient(circle at var(--r51-light-x) var(--r51-light-y),rgba(255,226,148,.095) 0,rgba(var(--r51-ambient-rgb),.055) 18%,transparent 42%),linear-gradient(180deg,rgba(var(--r51-ambient-rgb),.035),transparent 38%,rgba(0,0,0,.07));box-shadow:inset 0 0 78px rgba(0,0,0,.44);mix-blend-mode:screen;opacity:.78;transition:background .35s ease,opacity .2s ease";
    layer.classList.contains("r51-torch-live")&&(layer.style.opacity=".92");state.installs++;return true
  }
  function tick(){if(install()){const layer=document.getElementById("ccg-r51-world-lighting");if(layer)layer.style.opacity=layer.classList.contains("r51-torch-live")?".92":".78"}}
  tick();state.timer=setInterval(tick,700);addEventListener("pagehide",()=>{if(state.timer)clearInterval(state.timer)},{once:true});
