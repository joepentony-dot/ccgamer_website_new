...
(()=>{
  ...
  const state={timer:0,observer:null,installs:0};
  ...
  tick();state.timer=setInterval(tick,900);
  state.observer=new MutationObserver(()=>queueMicrotask(tick));
  state.observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:["data-run-active"]});
