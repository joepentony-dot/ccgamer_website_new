...
(()=>{
  ...
  const state={installTimer:0,lightingTimer:0,playerSource:null,playerWrapper:null,enemySource:null,enemyWrapper:null,playerFrames:0,enemyFrames:0,attackFx:0,hitFx:0,menuEnhanced:false,lightingUpdates:0,lastTheme:"",lastError:""};
  ...
  function drawPlayerBackdrop(p,kind){
    const s=playerScreen(p);if(!s)return;const col=kind==="p2"?(P?.green||"#72ff9b"):kind==="remote"?(P?.cyan||"#6cecff"):(P?.gold||"#ffd85a"),torch=num(p?.torchMs)>0;
    ...
    const moving=Math.abs(num(p?.x,p?.rx)-num(p?.rx))+Math.abs(num(p?.y,p?.ry)-num(p?.ry))>.025;if(moving&&!reduced()){
  ...
  function drawPlayerOverlay(p,kind){
    const s=playerScreen(p);if(!s)return;const col=kind==="p2"?(P?.green||"#72ff9b"):kind==="remote"?(P?.cyan||"#6cecff"):(P?.gold||"#ffd85a"),t=now(),swingMs=Math.max(1,num(p?._meleeSwingMs,260)),age=t-num(p?._meleeSwingAt,-Infinity),swing=age>=0&&age<swingMs;
    ctx.save();
    if(swing){const d=p?._meleeSwingDir||p?.dir||{x:1,y:0},base=Math.atan2(num(d.y),num(d.x,1)),progress=Math.max(0,Math.min(1,age/swingMs)),start=base-1.18+progress*.25,end=base+.72+progress*.45;ctx.globalAlpha=.52*(1-progress*.55);ctx.strokeStyle=p?._meleeSwingColour||p?.meleeWeapon?.colour||col;ctx.lineWidth=Math.max(2,s.tile*.055);ctx.shadowColor=ctx.strokeStyle;ctx.shadowBlur=10;ctx.beginPath();ctx.arc(s.x,s.y,s.tile*.48,start,end);ctx.stroke();ctx.globalAlpha=.24;ctx.lineWidth=1;ctx.strokeStyle="#fff";ctx.beginPath();ctx.arc(s.x,s.y,s.tile*.56,start+.08,end-.04);ctx.stroke();state.attackFx++}
    if(num(p?.hitStunMs)>0){ctx.globalAlpha=.42;ctx.strokeStyle="#fff";ctx.lineWidth=2;ctx.shadowColor=P?.red||"#ff6868";ctx.shadowBlur=14;ctx.beginPath();ctx.arc(s.x,s.y,s.tile*.42,0,Math.PI*2);ctx.stroke()}
    if(num(p?.dashMs)>0||num(p?._dashMs)>0){ctx.globalAlpha=.22;ctx.strokeStyle=col;ctx.lineWidth=3;ctx.beginPath();ctx.arc(s.x,s.y,s.tile*.38,0,Math.PI*2);ctx.stroke()}
    ctx.restore()
  }
  function playerTransform(p){
    if(reduced())return{sx:1,sy:1,dy:0};const t=now(),moving=Math.abs(num(p?.x,p?.rx)-num(p?.rx))+Math.abs(num(p?.y,p?.ry)-num(p?.ry))>.025,swingMs=Math.max(1,num(p?._meleeSwingMs,260)),age=t-num(p?._meleeSwingAt,-Infinity),swing=age>=0&&age<swingMs;
  ...
  function drawEnemyBackdrop(e){
    const s=enemyScreenPoint(e);if(!s)return;const col=enemyAccent(e),elite=Boolean(e?.follower||e?.champion||e?.guardian||e?.exitWarden||e?.deathStalker);ctx.save();const r=s.tile*(elite?.78:.5),g=ctx.createRadialGradient(s.x,s.y,2,s.x,s.y,r);g.addColorStop(0,`${col}${elite?"20":"10"}`);g.addColorStop(1,"rgba(0,0,0,0)");ctx.fillStyle=g;ctx.fillRect(s.x-r,s.y-r,r*2,r*2);
    if(e?.aiState==="chase"){ctx.globalAlpha=elite?.28:.12;ctx.strokeStyle=col;ctx.lineWidth=1;ctx.beginPath();ctx.ellipse(s.x,s.y+s.tile*.33,s.tile*(elite?.34:.27),s.tile*.11,0,0,Math.PI*2);ctx.stroke()}ctx.restore()
  }
  function drawEnemyOverlay(e){
    const s=enemyScreenPoint(e);if(!s)return;const col=enemyAccent(e),t=now();ctx.save();
    if(num(e?.flash)>0){const life=Math.max(.15,Math.min(1,num(e.flash)/180));ctx.globalAlpha=.72*life;ctx.strokeStyle="#fff";ctx.shadowColor=col;ctx.shadowBlur=10;ctx.lineWidth=1.4;for(let i=0;i<6;i++){const a=t*.01+i*Math.PI/3,r1=s.tile*.22,r2=s.tile*(.33+.06*((i+2)%3));ctx.beginPath();ctx.moveTo(s.x+Math.cos(a)*r1,s.y+Math.sin(a)*r1);ctx.lineTo(s.x+Math.cos(a)*r2,s.y+Math.sin(a)*r2);ctx.stroke()}state.hitFx++}
    if(e?.follower||e?.champion||e?.guardian||e?.exitWarden){const pulse=reduced()?.4:.28+.12*Math.sin(t/380);ctx.globalAlpha=pulse;ctx.strokeStyle=col;ctx.lineWidth=1;ctx.beginPath();ctx.arc(s.x,s.y,s.tile*.48,0,Math.PI*2);ctx.stroke()}
    ctx.restore()
  }
  function enemyTransform(e){
    if(reduced())return{sx:1,sy:1,dy:0};const t=now(),walking=e?.aiState==="chase"||e?.aiState==="search",seed=String(e?.id||e?.kind||"").length;
  ...
  function install(){installStylesheet();enhanceMenu();ensureLighting();updateLighting();const player=installPlayerWrapper(),enemy=installEnemyWrapper();document.body.dataset.v141R51Visual="true";return player&&enemy}
  install();state.installTimer=setInterval(install,850);state.lightingTimer=setInterval(updateLighting,100);
  addEventListener("pagehide",()=>{if(state.installTimer)clearInterval(state.installTimer);if(state.lightingTimer)clearInterval(state.lightingTimer);state.installTimer=state.lightingTimer=0},{once:true});
  window.CCGLostSizzlerV141R51VisualUIOverhaul={install,enhanceMenu,updateLighting,playerTransform,enemyTransform,get state(){return state}};
})();
