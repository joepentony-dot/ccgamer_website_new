...
(()=>{
  ...
  function applyServerState(server){
    ...
    const players=values(server.players).map(buildPlayer),enemyRows=values(server.enemies).map(buildEnemy),enemies=enemyRows.filter(row=>!row.boss),boss=enemyRows.find(row=>row.boss)||null,pickups=values(server.pickups).map(row=>({id:String(row.id),x:Number(row.x)||0,y:Number(row.y)||0,restore:Number(row.restoreAmount||2)}));
