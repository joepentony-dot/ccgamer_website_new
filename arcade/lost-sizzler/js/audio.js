window.CCGSound=(()=>{
  ...
  function musicTick(){
    ...
    if(roomMood==="named"){
      const n=namedMelody[step%namedMelody.length];if(n){tone(n,.16,"triangle",.12,12,0,"music");tone(n/2,.24,"sine",.08,-3,.02,"music")}if(step%3===2)noise(.025,.01,"music");step++;return;
    }
    if(roomMood==="danger"){
      const n=dangerMelody[step%dangerMelody.length];if(n){tone(n,.17,"triangle",.11,-6,0,"music");if(step%2===0)tone(n/2,.25,"sine",.09,0,.02,"music")}if(step%4===2)noise(.028,.009,"music");step++;return;
    }
    ...
    const n=melody[step%melody.length];if(n)tone(n,.15,"triangle",.15,0,0,"music");if(step%4===0)tone(bass[Math.floor(step/4)%bass.length],.28,"sine",.15,0,0,"music");const c=counter[step%counter.length];if(c&&step%2===1)tone(c,.08,"sine",.045,-7,0,"music");if(step%8===4)noise(.018,.006,"music");step++
  }
