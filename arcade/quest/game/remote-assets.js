(function(){
  ...
  Q.hydrateRemoteAssets=async function(){
    ...
    try{
      await loadScript('/js/ccg-supabase-config.js');
      await loadScript('/js/ccg-supabase-client.js');
      const client=await waitForClient();
      if(!client)return false;

      const {data,error}=await client
        .from('arcade_assets')
        .select('asset_group,asset_key,public_url,enabled,asset_meta')
        .eq('enabled',true);

      if(error){
        console.info(TAG,'remote asset manifest unavailable; bundled assets remain active',error.message);
      }else{
      ...
      console.info(TAG,'using bundled assets only',error?.message||error);
      return false;
    }
  };
})();