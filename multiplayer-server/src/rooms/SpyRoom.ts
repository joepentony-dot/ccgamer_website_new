...
const cleanName = (value: unknown) => String(value ?? "Agent").trim().slice(0, 18) || "Agent";
...
export class SpyRoom extends Room {
  maxClients = 2;
  ...
  onCreate(options: SpyJoinOptions = {}) {
    this.roomCode = cleanCode(options.roomCode) || cleanCode(this.roomId.slice(-6));
    this.setMetadata({ mode: "spy", roomCode: this.roomCode, playerCap: 2, transport: "colyseus" });

    this.onMessage("game", (client, message: SpyGameMessage) => this.relayGame(client, message));
  ...
  onJoin(client: Client, options: SpyJoinOptions = {}) {
  ...
  private relayGame(client: Client, message: SpyGameMessage) {
    const actorId = this.actorFor(client);
    const event = cleanEvent(message?.event);
    if (!actorId || !event) return;

    // Transport-only migration: the browser-host Spy rules engine and the
    // retained r29-r35 hardening layers continue to own gameplay authority.
    // Colyseus carries their existing packets unchanged and does not echo a
    // client's packet back to itself, matching the old Supabase self:false path.
