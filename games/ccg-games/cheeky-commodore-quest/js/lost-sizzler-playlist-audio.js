(()=>{
   ...
   * V10.7 layer loads. Mutate that existing object in place so the cached game
  ...
  function categorySources(state){
    ...
    const custom=groups.custom.filter(url=>!failedUrls.has(url));
    if(custom.length)return custom;
    return groups.bundled.filter(url=>!failedUrls.has(url));
  }
  ...
  function parkExploration(slot){
    ...
    try{slot.audio.pause();slot.audio.volume=targetVolume("normal")}catch(_){}
  ...
  function failSlot(slot){
    if(!slot)return;
    failedUrls.add(slot.url);
    slot.advancing=true;
    if(slot===explorationSlot)explorationSlot=null;
    if(slot===fadingOut)fadingOut=null;
    if(current===slot){
      current=null;
      destroySlot(slot);
      setTimeout(()=>transition(true,true),120);
  ...
  function armAdvance(slot){
    ...
    slot.audio.addEventListener("ended",()=>{if(current===slot&&!slot.advancing){slot.advancing=true;transition(true,true)}});
    slot.audio.addEventListener("error",()=>failSlot(slot));
  }
  ...
  function resumeExploration(previous){
    ...
    try{
      ...
      Promise.resolve(play).then(()=>fadeBetween(previous,next)).catch(()=>{
        ...
        if(current===next){explorationSlot=null;current=previous;destroySlot(next);transition(true,true)}
      });
      return true;
    }catch(_){
      failedUrls.add(next.url);
  ...
  function transition(force=false,advance=false){
    ...
    if(state==="normal"&&!advance){
      if(current?.state==="normal"){
        ...
        if(current.audio.paused)current.audio.play().catch(()=>failSlot(current));
        updateVolume();
        return;
      }
    ...
    if(current&&current.state===state&&!current.audio.paused&&!force){updateVolume();return}
    ...
    const next=makeSlot(state,url),previous=current;
    current=next;
    ...
    next.audio.play().then(()=>fadeBetween(previous,next)).catch(()=>{
      ...
      if(current===next){
        ...
        current=previous;
        destroySlot(next);
        if(previous&&!previous.audio.paused)updateVolume();
        else setTimeout(()=>transition(true,true),120);
      }
    });
  }
  ...
  function stopMusic(){
    ...
    current=null;fadingOut=null;explorationSlot=null;
    try{original.stopMusic?.()}catch(_){}
  }

  function setRoomMood(value){
    const next=normaliseState(value);
    if(next===roomMood){if(next==="normal")transition(false,false);return}
    roomMood=next;
  ...
  function setMusicLevel(value){musicLevel=Math.max(0,Math.min(.25,Number(value)||0));updateVolume();if(explorationSlot&&explorationSlot!==current)explorationSlot.audio.volume=targetVolume("normal")}
  ...
  function windWhistle(...args){const result=original.windWhistle?.(...args);setTimeout(()=>{try{original.stopMusic?.()}catch(_){}},0);return result}

  try{original.stopMusic?.()}catch(_){}

  Object.assign(base,{
    start,startMusic,stopMusic,toggle,isEnabled:()=>enabled,setMusicLevel,setRoomMood,
    setNamedEnemy:()=>{},setStalkerNear,setStalkerSight,setDanger:()=>{},
    sfx:(name,...args)=>name==="room"?undefined:original.sfx?.(name,...args),windWhistle
  });
  window.CCGSound=base;

  window.CCGLostSizzlerPlaylistAudio={
    refresh:()=>transition(false,false),
    getState:()=>({state:desiredState(),url:current?.url||"",enabled,started,explorationUrl:explorationSlot?.url||"",explorationTime:Number(explorationSlot?.audio?.currentTime||0),explorationParked:Boolean(explorationSlot&&explorationSlot!==current),failed:[...failedUrls]}),
    getPlaylist:state=>categorySources(normaliseState(state)),
    clearFailed:()=>failedUrls.clear(),
    crossfadeMs:FADE_MS
  };

  /* Newly uploaded sources become eligible without restarting the current song. */
