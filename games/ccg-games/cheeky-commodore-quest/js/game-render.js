...
function drawPixelEnemySprite(e,cx,cy){
  ...
  if(!["firebreather","ghost"].includes(k)){px(0,0,-8+legA,8,6,9,pal.dark);px(0,0,2+legB,8,6,9,pal.dark);px(0,0,-10+legA,15,8,3,pal.outline);px(0,0,2+legB,15,8,3,pal.outline)}
  if(k==="scout"){
...
function drawFurniture(){
  for(const d of world.decor||[]){
    ...
    ctx.globalAlpha=.52;ctx.fillStyle="rgba(255,255,255,.15)";ctx.fillRect(q.x+6,q.y+6,Math.max(4,C.tile-12),1);if(h%3===0){ctx.strokeStyle="rgba(10,5,13,.42)";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(q.x+8+(h%9),q.y+9);ctx.lineTo(q.x+16+(h%12),q.y+18);ctx.stroke()}if(d.blocking){ctx.fillStyle="rgba(0,0,0,.28)";ctx.fillRect(q.x+4,q.y+C.tile-5,C.tile-8,3);ctx.fillStyle="#c4a35c";ctx.fillRect(q.x+6,q.y+C.tile-7,2,2);ctx.fillRect(q.x+C.tile-8,q.y+C.tile-7,2,2)}ctx.globalAlpha=1;ctx.restore()
  }
}
...
function drawFog(){const ex=explored.get(focus.id)||new Set(),x0=Math.max(0,Math.floor(cam.x/C.tile)-1),x1=Math.min(C.worldWidth-1,Math.ceil((cam.x+view.w)/C.tile)+1),y0=Math.max(0,Math.floor(cam.y/C.tile)-1),y1=Math.min(C.worldHeight-1,Math.ceil((cam.y+view.h)/C.tile)+1),torchEnemies=(host.enemies||[]).filter(e=>e.alive&&e.follower),wallLights=world.wallLights||[];for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){if(visibleTo(focus,x,y))continue;const s=ws(x,y);let alpha=ex.has(`${x},${y}`) ? .92 : 1;for(const e of torchEnemies){const d=Math.hypot(x-e.x,y-e.y),r=C.enemy.followerLightRadius||10;if(d<=r)alpha=Math.min(alpha,.32+(d/r)*.48)}for(const l of wallLights){const d=Math.hypot(x-l.x,y-l.y),r=l.radius||5;if(d<=r)alpha=Math.min(alpha,.45+(d/r)*.4)}if(host.exitOpen){const d=Math.hypot(x-world.exit.x,y-world.exit.y);if(d<=5)alpha=Math.min(alpha,.38+d/5*.45)}ctx.fillStyle=`rgba(0,0,0,${alpha})`;ctx.fillRect(s.x,s.y,C.tile,C.tile)}drawDynamicLighting()}
