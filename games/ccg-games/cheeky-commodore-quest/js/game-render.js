...
function drawSpecialObjects(){
  ...
  for(const c of host.deathCaches||[])if(c.active&&visibleTo(focus,c.x,c.y)){const s=ws(c.x,c.y);ctx.save();ctx.fillStyle="#2a1c31";ctx.strokeStyle=P.red;ctx.lineWidth=2;ctx.shadowColor=P.red;ctx.shadowBlur=9;ctx.fillRect(s.x+5,s.y+8,C.tile-10,C.tile-12);ctx.strokeRect(s.x+5,s.y+8,C.tile-10,C.tile-12);ctx.fillStyle=P.white;ctx.fillRect(s.x+10,s.y+12,C.tile-20,3);ctx.fillStyle=P.red;ctx.fillRect(s.x+C.tile/2-2,s.y+18,4,5);ctx.restore();if(md(c,focus)<=3)label("DEATH CACHE — RECOVER SCORE & LOOT",s,P.red)}
