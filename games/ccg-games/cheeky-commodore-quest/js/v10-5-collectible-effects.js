...
(function(){
  ...
  function overhead(player,text,colour=P.gold){if(!player||!text)return;setTimeout(()=>{try{floatText(player.x,player.y,String(text).toUpperCase(),colour)}catch(_){}},420)}
  ...
  function casinoRoll(player){const roll=Math.floor(Math.random()*5);if(roll===0){score+=1200;toast("CASINO WIN","Jackpot: +1,200 score.","gold")}else if(roll===1){score=Math.max(0,score-500);toast("CASINO LOSS","The house wins: -500 score.","red")}else if(roll===2){awardXP(player,300,"Casino collectible");toast("CASINO XP WIN","+300 XP.","cyan")}else if(roll===3){player.armor=Math.min(12,Number(player.armor||0)+3);toast("CASINO ARMOUR WIN","+3 armour.","green")}else{player.mana=Math.min(player.maxMana,player.mana+35);player.ammoFlashMs=C.player.ammoFlashMs;toast("CASINO AMMO WIN","+35 ammunition.","cyan")}}
  function colourBurst(player){const roll=Math.floor(Math.random()*3);if(roll===0){player.health=Math.min(player.maxHealth,player.health+2);player.hpBarMs=2800;toast("WIZBALL COLOUR — GREEN","+2 health.","green")}else if(roll===1){player.mana=Math.min(player.maxMana,player.mana+30);player.ammoFlashMs=C.player.ammoFlashMs;toast("WIZBALL COLOUR — CYAN","+30 ammunition.","cyan")}else{player.armor=Math.min(12,Number(player.armor||0)+2);toast("WIZBALL COLOUR — BLUE","+2 armour.","cyan")}}
  function wildcard(player){const roll=Math.floor(Math.random()*4);if(roll===0){awardXP(player,250,"Wildcard collectible");toast("WILDCARD — XP","+250 XP.","cyan")}else if(roll===1){score+=750;toast("WILDCARD — SCORE","+750 score.","gold")}else if(roll===2){player.rapidMs=Math.max(player.rapidMs||0,7000);toast("WILDCARD — RAPID FIRE","Seven seconds of rapid fire.","gold")}else{safeHop(player);toast("WILDCARD — WARP","You have been bounced somewhere else on the floor.","purple")}}
  ...
  function applyRule(rule,player,title){
    ...
    switch(rule.effect_type){
      ...
      case"arcade_frenzy":player.rapidMs=Math.max(player.rapidMs||0,duration);player.mana=Math.min(player.maxMana,player.mana+Number(config.ammo||15));player.ammoFlashMs=C.player.ammoFlashMs;toast(`${title} — ARCADE FRENZY`,`Rapid fire plus ${Number(config.ammo||15)} ammo for the pickup burst.`,"gold");break;
      ...
      case"xp_boon":awardXP(player,Number(config.xp||300),`${title} RPG bonus`);toast(`${title} — XP BOOST`,`+${Number(config.xp||300)} XP, subject to the floor level cap.`,"cyan");break;
      case"brain_boost":awardXP(player,Number(config.xp||150),`${title} knowledge bonus`);temp(player,"brain_boost",duration,config);toast(`${title} — BRAIN BOOST`,`+${Number(config.xp||150)} XP and sharper sight for ${Math.ceil(duration/1000)} seconds.`,"cyan");break;
      case"weapon_overdrive":player.rapidMs=Math.max(player.rapidMs||0,duration);player.mana=Math.min(player.maxMana,player.mana+Number(config.ammo||25));player.ammoFlashMs=C.player.ammoFlashMs;toast(`${title} — WEAPON OVERDRIVE`,`Rapid fire and +${Number(config.ammo||25)} ammo.`,"gold");break;
      ...
      case"turbo_blaster":temp(player,"turbo_blaster",duration,config);player.rapidMs=Math.max(player.rapidMs||0,duration);player.mana=Math.min(player.maxMana,player.mana+Number(config.ammo||30));player.ammoFlashMs=C.player.ammoFlashMs;toast(`${title} — TURBO BLASTER`,`Speed, rapid fire and +${Number(config.ammo||30)} ammo for ${Math.ceil(duration/1000)} seconds.`,"gold");break;
