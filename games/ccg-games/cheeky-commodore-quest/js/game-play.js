...
function triggerDeathCache(p){for(const c of host.deathCaches||[])if(c.active&&c.x===p.x&&c.y===p.y){const r=PGR.recoverDeathCache(p,run,c);score+=r.score||0;S.sfx("pickup");host.revision++;showToast("DEATH CACHE RECOVERED",`${r.recovered} carried item${r.recovered===1?"":"s"}, ${r.games} rescued game${r.games===1?"":"s"} and ${Number(r.score||0).toLocaleString()} score recovered${r.remaining?`. ${r.remaining} item${r.remaining===1?"":"s"} remain because your inventory is full.`:"."}`,r.remaining?"cyan":"green",8500);broadcastWorld()}}
