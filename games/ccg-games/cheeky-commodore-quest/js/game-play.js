...
function tryDoor(p,x,y){
  ...
  if(d.type==="secret"){
    d.locked=false;d.hidden=false;stats.secrets++;run.stats.secrets++;S.sfx("secret");awardXP(p,180,"Secret room discovered");
  ...
  p.bronzeKeys--;d.locked=false;stats.doors++;shake=5;awardXP(p,60,"Bronze door opened");
...
function triggerRescue(p){const r=host.rescue;if(!r||r.rescued)return;if(!r.following&&md(p,r)<=1){r.following=true;r.found=true;awardXP(p,160,"Trapped scout found");showToast("CCG SCOUT FOUND","Escort the scout itself into one of the permanently lit sanctuary rooms. It follows the nearest player.","green",9000)}}
...
function triggerDeathCache(p){for(const c of host.deathCaches||[])if(c.active&&c.x===p.x&&c.y===p.y){const r=PGR.recoverDeathCache(p,run,c);S.sfx("pickup");host.revision++;showToast("DEATH CACHE RECOVERED",`${r.recovered} carried item${r.recovered===1?"":"s"} and ${r.games} rescued game${r.games===1?"":"s"} recovered${r.remaining?`. ${r.remaining} item${r.remaining===1?"":"s"} remain because your inventory is full.`:"."}`,r.remaining?"cyan":"green",8500);broadcastWorld()}}
...
function triggerBloodClue(p){const c=host.bloodClue;if(!c||c.seen||md(c,p)>1)return;c.seen=true;run.torchClueSeen=true;run.torchSequence=[...(c.sequence||[])];host.revision++;S.sfx("secret");showToast("FADED BLOOD CLUE",`Four marks remain legible: ${(c.sequence||[]).join(" → ")}. It looks less like decoration and more like an instruction. TAB will keep the clue in your log.`,"red",11000);awardXP(p,90,"Faded blood clue found");broadcastWorld()}
...
function solveSequenceTorch(p){const z=host.sequenceTorchPuzzle;if(!z||z.solved)return;z.solved=true;z.progress=z.sequence.length;for(const t of z.torches||[])t.lit=true;const chest=(host.chests||[]).find(c=>c.id===z.chestId);if(chest)chest.active=true;score+=650;awardXP(p,360,"Sequence torch vault opened");S.sfx("open");showToast("TORCH VAULT OPEN","All four flames hold. Stone grinds aside and the vault reward is exposed.","gold",9500);host.revision++;broadcastWorld()}
...
function damageGenerator(g,power,p){
  ...
  g.alive=false;run.stats.generators++;stats.generators++;score+=700;awardXP(p,520,"Monster generator destroyed");let artefact=false;
  ...
  showToast("GENERATOR DESTROYED",`Reinforcements from this machine have stopped. +700 score and +520 XP${artefact?". Its core dropped a rare artefact.":"."}`,"green",8500);SYS.updateObjective(host,run,Math.round(PGR.roomCompletion(explored.get(p.id)||new Set(),world)*100));host.revision++;broadcastWorld()
}
...
function hurtPlayer(p,n,friendly=false,source="enemy"){
  ...
  if(p.health<=0){
    ...
    const explore=Math.round(PGR.roomCompletion(explored.get(p.id)||new Set(),world)*100),objective=SYS.objectiveText(host,run,explore),cacheText=cache.active?" Your carried loot is in a death cache where you fell.":" You had no carried loot to cache.",xpText=penalty.xpLost?` ${penalty.xpLost} XP lost; ${penalty.xpAfter} XP reserve remains.`:" No XP has been earned yet, so the XP game-over rule is not active.";
...
function updateCamping(p,dt){let c=campStates.get(p.id);if(!c){resetCamp(p);c=campStates.get(p.id)}if(p.x!==c.lastX||p.y!==c.lastY){resetCamp(p,true);return}c.elapsed+=dt;if(c.elapsed<C.camping.graceMs)return;if(!c.active){c.active=true;c.nextBlast=150;c.blastCount=0;S.sfx("campwarn");run.alert=Math.min(100,run.alert+14);showToast("60 SECONDS IDLE — MOVE NOW","You have been completely stationary for one minute. Move to cancel the targeted blasts.","red",7000)}c.nextBlast-=dt;if(c.nextBlast<=0){c.blastCount++;let q;if(c.blastCount%C.camping.directBlastEvery===0)q={x:p.x,y:p.y,direct:true};else{const a=[];for(let dy=-C.camping.zoneRadius;dy<=C.camping.zoneRadius;dy++)for(let dx=-C.camping.zoneRadius;dx<=C.camping.zoneRadius;dx++){const x=p.x+dx,y=p.y+dy;if(W.walkable(world.map,x,y,host)&&Math.hypot(dx,dy)<=C.camping.zoneRadius+.2)a.push({x,y})}q=a[Math.floor(Math.random()*a.length)]||{x:p.x,y:p.y}}hazards.push({x:q.x,y:q.y,life:C.camping.warningMs,maxLife:C.camping.warningMs,direct:!!q.direct,campOwner:p.id});c.nextBlast=C.camping.blastIntervalMs}}
function updateHazards(dt){for(let i=hazards.length-1;i>=0;i--){const h=hazards[i];h.life-=dt;if(h.life>0)continue;hazards.splice(i,1);S.sfx("explosion");shake=Math.max(shake,h.direct?13:9);burst(h.x,h.y,P.orange,h.direct?32:24,h.direct?2.1:1.8);ring(h.x,h.y,P.red,h.direct?54:42);for(const p of localPlayers())if(Math.hypot(p.x-h.x,p.y-h.y)<=C.camping.blastRadius)hurtPlayer(p,C.camping.damage,false,"anti-loitering blast")}}
