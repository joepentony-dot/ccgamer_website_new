...
(()=>{
  ...
  const state={forceTutorial:false,choiceAccepted:false,tutorialRequested:false,active:false,step:0,start:null,moved:false,fired:false,dashed:false,inventoryOpened:false,pendingBegin:null,lockedDoors:[],panel:null,choice:null,welcomeForRun:false,installed:{begin:false,startWorld:false,actions:false,dossier:false}};
  ...
  function installBegin(){if(state.installed.begin||typeof beginRun!=="function")return;const original=beginRun;beginRun=function(opts={}){const daily=Boolean(opts?.daily),online=Boolean(opts?.online);if(!state.choiceAccepted&&!daily&&!online&&(state.forceTutorial||!readFlag(SEEN))){showChoice(Array.from(arguments));return false}const result=original.apply(this,arguments);afterRunStarted(daily);return result};state.installed.begin=true}
  ...
  function installActions(){
    if(state.installed.actions)return;let n=0;
    if(typeof firePlayer==="function"&&!firePlayer.__tutorial){const o=firePlayer;firePlayer=function(){const r=o.apply(this,arguments);note("fire");return r};firePlayer.__tutorial=true;n++}
    if(typeof dashPlayer==="function"&&!dashPlayer.__tutorial){const o=dashPlayer;dashPlayer=function(){const r=o.apply(this,arguments);note("dash");return r};dashPlayer.__tutorial=true;n++}
    if(typeof toggleInventory==="function"&&!toggleInventory.__tutorial){const o=toggleInventory;toggleInventory=function(){const r=o.apply(this,arguments);try{if(UI?.inventory&&!UI.inventory.classList.contains("hidden"))note("inventory")}catch(_){}return r};toggleInventory.__tutorial=true;n++}
    if(typeof hurtPlayer==="function"&&!hurtPlayer.__tutorial){const o=hurtPlayer;hurtPlayer=function(){if(state.active)return false;return o.apply(this,arguments)};hurtPlayer.__tutorial=true;n++}
    state.installed.actions=n>=4;
  }
