...
(()=>{
  ...
  const state={forceTutorial:false,choiceAccepted:false,tutorialRequested:false,active:false,step:0,start:null,moved:false,movementDistance:0,lastMovement:new Map(),fired:false,dashed:false,inventoryOpened:false,inventoryClosed:false,pendingBegin:null,lockedDoors:[],panel:null,choice:null,welcomeForRun:false,autoAdvanceTimer:0,installed:{begin:false,startWorld:false,actions:false,dossier:false}};
  ...
  function stepReady(s){if(!s)return true;if(s[0]==="move")return state.moved;if(s[0]==="fire")return state.fired;if(s[0]==="dash")return state.dashed;if(s[0]==="inventory")return state.inventoryOpened&&state.inventoryClosed;return true}
  ...
  function completeInteractive(kind){
    ...
    if(kind==="move")state.moved=true;
    if(kind==="fire")state.fired=true;
  ...
  function watchTutorialProgress(){
    ...
    if(STEPS[state.step]?.[0]==="move"){
      for(const p of typeof localPlayers==="function"?localPlayers():[p1,p2].filter(Boolean)){
        const key=p.id||p,prev=state.lastMovement.get(key)||{x:Number(p.x||0),y:Number(p.y||0)},now={x:Number(p.x||0),y:Number(p.y||0)},d=Math.abs(now.x-prev.x)+Math.abs(now.y-prev.y);state.lastMovement.set(key,now);if(d>0){state.movementDistance+=d;if(state.movementDistance>=2){completeInteractive("move");break}}
      }
    }
  ...
  window.CCGLostSizzlerOnboardingV120={state,replay:()=>{state.forceTutorial=true;state.tutorialRequested=true;return typeof startSolo==="function"?startSolo():false},isTutorialComplete:()=>readFlag(COMPLETE),markVerifiedEnemy:markVerified,stepReadyForTest:stepReady,completeInteractiveForTest:completeInteractive};
})();