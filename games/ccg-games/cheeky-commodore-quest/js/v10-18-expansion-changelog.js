...
(()=>{
  ...
  const entries=[
    ...
    ["LS-0823-33","FIXED","Tutorial stage progression","The Training Archive now counts cumulative successful movement instead of only distance from the original start tile. Movement, firing and dash stages advance automatically after the required action, while the inventory stage verifies both opening and closing the panel. Fast polling and self-healing action hooks cover keyboard, gamepad, touch and later control wrappers."],
    ["LS-0823-34","ADDED","Build and cache update checker","The main menu now checks its loaded Lost Sizzler build against a no-cache live version manifest. An older cached build is identified to the player with a Refresh to Latest Version action, while core game JavaScript and CSS now use release cache tokens so a refresh cannot simply reload stale runtime files."],
    ["LS-0823-35","FIXED","Tutorial start and completion flow","Every normal game start now asks the player to choose TUTORIAL or PLAY GAME. Movement, fire, dash and inventory stages advance from the required action with direct keyboard and touch fallbacks, informational stages continue in sequence, and finishing training now reports completion and returns to the main options instead of dropping the player straight into a live dungeon run."],
    ["LS-0823-36","FIXED","Tutorial mobile guidance and automatic explanations","Tutorial control stages now flash the relevant movement, FIRE, DASH or ITEMS control while waiting for the player's input. Explanatory stages no longer require a hidden key or button press: they remain readable for a short timed section and then continue automatically, with an optional Continue Now control. On phones the tutorial temporarily expands the report rail so its instructions are not clipped by the normal 50px gameplay-notification limit."]
  ];
