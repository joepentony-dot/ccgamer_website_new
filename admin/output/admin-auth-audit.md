...
## Final Admin Auth Fix — No More Loops

- ✅ Removed legacy files: `admin/admin-backup.html`, `admin/admin-backup.js`.
- ✅ Confirmed no `admin/*.html`, `admin/js/*.js`, or `js/*.js` references to those deleted files.
- ✅ `admin/js/guard.js` now waits for Supabase client/session before redirecting and does not issue forbidden bootstrap redirects.
- ✅ `admin/js/login.js` no longer performs guard/session restore checks; it only signs in and redirects to `/admin/dashboard.html`.
- ✅ `admin/js/admin-nav.js` logout link is standardized with `data-logout` and loads shared logout binding script.
- ✅ `js/ccg-auth-ui.js` now binds logout only via `[data-logout]` and redirects to `/auth/login.html?reason=signed_out`.
- ✅ `admin/js/dashboard.js` no longer performs direct auth redirects; access control is delegated to `guard.js`.
