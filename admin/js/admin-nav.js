...
export async function initAdminNav({ pageLabel = "Dashboard", active = "dashboard" } = {}) {
  ...
  shell.innerHTML = `
    <div class="omega-admin-bar">
      <div class="omega-admin-brand"><strong>CCG ADMIN PANEL</strong><span>${escapeHtml(pageLabel)}</span></div>
  ...
  host.prepend(shell);
  const activeLink = shell.querySelector(`[data-nav="${active}"]`);if (activeLink) activeLink.classList.add("is-active");
  bindLogout(shell);
  const sessionNode = shell.querySelector("[data-admin-session]");
  try {
    await waitForAuthReady();
    const context = await getAuthContext();
    if (!context?.session?.user) { sessionNode.textContent = "Session: guest"; return; }
  ...
  } catch { sessionNode.textContent = "Session status unavailable."; }
}

export function injectDeprecatedBanner(message = "Legacy admin page") {
  const existing = document.querySelector(".omega-deprecated-banner");if (existing) return;
  const banner = document.createElement("aside");banner.className = "omega-deprecated-banner";
  ...
  const parent = document.querySelector(".ccg-page") || document.body;parent.prepend(banner);
}
