...
function buildFlatGamePages(games) {
  const pages = [];
  const warnings = [];
  const seenSlugs = new Set();

  if (!Array.isArray(games)) {
    warnings.push('games.json payload missing or invalid — no stubs generated.');
  ...
  games.forEach((game, index) => {
    ...
    const label = game.title || game.id || `Entry ${index + 1}`;
    if (!slug) {
      warnings.push(`Missing slug for "${label}" — flat page not generated.`);
      return;
    }
    const slugKey = slug.toLowerCase();
    if (seenSlugs.has(slugKey)) {
      warnings.push(`Duplicate slug "${slug}" detected — flat page skipped.`);
      return;
    }
    seenSlugs.add(slugKey);
    try {
      const html = buildGamePageHtml(game, { siteOrigin: SITE_ORIGIN });
      pages.push({ slug, html });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      warnings.push(`Failed to build flat page for "${slug}": ${message}`);
    }
  });
...
async function downloadFlatPageZip({ autoDownload = true } = {}) {
  if (state.export.disabled) {
    throw new Error(state.export.reason || 'Export disabled');
  }
  const { pages, warnings } = buildFlatGamePages(state.library);
  if (!pages.length) {
    throw new Error('No games with slugs found — flat page export aborted.');
  }

  const zip = new JSZip();
  zip.file(
    'README.txt',
    'Extract into /games/ (repo root). These are flat SEO game pages generated by the admin export.'
  );
