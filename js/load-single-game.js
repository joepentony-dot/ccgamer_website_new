...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    let gameSlugRaw = decodeURIComponent(
        (params.get("slug") || params.get("game") || "").toString().trim()
    );
    if (!gameSlugRaw) {
        gameSlugRaw = getSlugFromPath();
    }
    const gameSlug = normaliseGameSlugInput(gameSlugRaw);
    ...
    try {
        ...
        if (gameSlug) {
            game = CCG_SINGLE_ALL_GAMES.find(g => {
                ...
                const titleSlug = resolveGameSlug(title);
                const idDash = id.replace(/_/g, "-").toLowerCase();
                return (
                    idSlug === gameSlug ||
                    titleSlug === gameSlug ||
