/* ============================================================
   CCG MUSIC ARCHIVE SHARED SHELL
   ------------------------------------------------------------
   Adds the established CCG header, mobile drawer, mode control
   and account/navigation modules to Music Hub and composer
   pages without rewriting curated composer documents.
============================================================ */

(function () {
    "use strict";

    if (window.CCG_MUSIC_SHELL_READY) return;
    window.CCG_MUSIC_SHELL_READY = true;
    ...
    function ensureStylesheet(href) {
        ...
        link.rel = "stylesheet";
        link.href = href;
        link.dataset.ccgMusicShell = "true";
        document.head.appendChild(link);
    }

    function ensureScript(src) {
        const exists = Array.from(document.scripts)
            .some((script) => normalizePath(script.getAttribute("src")) === src);
        if (exists) return;
        const script = document.createElement("script");
        script.src = src;
        script.defer = true;
        script.dataset.ccgMusicShell = "true";
            ...
            <header class="ccg-header" data-ccg-header>
                <div class="ccg-header-inner">
                    <a href="/home.html" class="ccg-brand">
                        <img src="/resources/images/ccgamer-logo.png" alt="Cheeky Commodore Gamer logo" class="ccg-brand__logo" loading="eager" width="1500" height="1032">
                    ...
                    <nav class="ccg-nav" id="ccg-primary-nav" aria-label="Primary navigation">
                ...
                <div class="ccg-nav-drawer" data-ccg-nav-drawer aria-hidden="true">
                    <div class="ccg-nav-drawer__backdrop" data-ccg-drawer-close></div>
                    <div class="ccg-nav-drawer__panel" role="dialog" aria-modal="true" aria-label="Navigation menu">
                        <div class="ccg-nav-drawer__header">
                            ...
                            <button class="ccg-nav-drawer__close" type="button" data-ccg-drawer-close aria-label="Close menu">✕</button>
                        </div>
    ...
    function injectHeader() {
        if (document.querySelector("[data-ccg-header]")) return document.querySelector("[data-ccg-header]");
        const template = document.createElement("template");
        template.innerHTML = headerMarkup().trim();
        const header = template.content.firstElementChild;
        const firstContent = document.querySelector("main, .ccg-page") || document.body.firstElementChild;
        if (firstContent) document.body.insertBefore(header, firstContent);
        else document.body.prepend(header);
        document.body.classList.add("ccg-music-shared-shell");
        return header;
    }
    ...
    function init() {
        STYLESHEETS.forEach(ensureStylesheet);
