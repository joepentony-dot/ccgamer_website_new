...
test("single-game community runtime uses compact aggregate/read RPCs and preserves write paths", () => {
  ...
  const hardening = fs.readFileSync("supabase/migrations/20260922030000_single_game_community_review_hardening.sql", "utf8");
  ...
  assert.ok(
    comments.indexOf("const sortSelect = document.getElementById('ccg-review-sort')") <
      comments.indexOf("if (!user) {\n      const loginBtn = document.getElementById('ccg-login-to-comment')"),
    "guest review sort/pagination handlers must bind before the guest early return"
  );

  assert.match(migration, /security invoker/gi);
  assert.match(migration, /create table if not exists public\.comment_helpful_votes/);
  assert.match(hardening, /create or replace function public\.ccg_current_user_not_soft_banned\(\)/);
  assert.match(hardening, /create or replace function public\.ccg_game_reviews[\s\S]*security definer/i);
  assert.match(hardening, /comment_helpful_votes_owner_insert[\s\S]*ccg_current_user_not_soft_banned\(\)/);
