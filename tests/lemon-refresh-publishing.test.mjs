...
test("changed and discovered Lemon sources are resolved before magazine import", () => {
  ...
  const discoveryStep = workflow.match(/- name: Discover Lemon source for newly added games \(best effort\)[\s\S]*?continue-on-error:\s*true[\s\S]*?node scripts\/discover-new-lemon-source\.js --base HEAD\^/);
  ...
  assert.ok(discoveryIndex >= 0, "automatic source discovery for new games is missing");
  ...
  assert.ok(discoveryIndex < missingRefreshIndex, "automatic discovery must complete before fallback cache retry");
  ...
  assert.ok(discoveryStep, "automatic source discovery must remain best-effort so an external Lemon outage cannot block publishing");
});
