...
const responses = [
  ...
  new Response(JSON.stringify({
    ...
    purchase_units: [{
      ...
      payments: { captures: [{ id: 'CAPTURE123', status: 'COMPLETED', amount: { currency_code: 'GBP', value: '1.99' } }] },
    }],
  }), { status: 201 }),
];
...
assert.deepEqual(gateway.publicConfig(), { environment: 'sandbox', clientId: 'merchant-client-id' });
...
assert.equal(createBody.purchase_units[0].reference_id, 'the-lost-sizzler-full-game');
...
assert.equal(createBody.purchase_units[0].amount.value, '1.99');
...
assert.throws(() => createPayPalOrdersGateway({ environment: 'production', clientId: 'x', clientSecret: 'y', fetchImpl }), /sandbox or live/);
...
console.log('C64 Dungeon Carnage PayPal Orders contract passed.');
