...
export function createLostSizzlerCommerceService({
  ...
  if (!database?.query || !database?.transaction) throw new Error('C64 Dungeon Carnage commerce requires a database.');
  if (commerceEnabled && (!gateway?.createOrder || !gateway?.captureOrder || !gateway?.publicConfig)) {
    throw new Error('Enabled C64 Dungeon Carnage commerce requires a PayPal Orders gateway.');
  }
  if (typeof randomUuidImpl !== 'function') throw new Error('C64 Dungeon Carnage commerce requires a UUID source.');
  ...
  return Object.freeze({
    async offer() {
      ...
      return Object.freeze({
        ...
        message: 'Unlock C64 Dungeon Carnage permanently. Your purchase is tied to your CCG account and includes all future game updates at no extra charge.',
      });
    },
    ...
    async createOrder(userIdValue) {
      ...
      try {
        providerOrder = await gateway.createOrder({
          ...
          description: 'C64 Dungeon Carnage — permanent full-game unlock including all future updates',
        });
      } catch (error) {
